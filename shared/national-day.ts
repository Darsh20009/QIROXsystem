export const NATIONAL_DAY_CAMPAIGN = {
  ...
  discountRate: 0.096,
  discountPercent: 9.6,
  message: "دام عزك يا وطن 🇸🇦",
  subtitle: "احتفالًا باليوم الوطني السعودي",
  title: "خصم اليوم الوطني",
  offer: "على جميع الباقات",
  cta: "اكتشف الباقات",
  flagSrc: "/saudi-flag-waving.png",
  compactFlagSrc: "/saudi-flag-fabric.png",
  flagAlt: "علم المملكة العربية السعودية",
} as const;

export const NATIONAL_DAY_PLAN_PRICES = {
...
export type NationalDayTier = "lite" | "pro" | "infinity" | "infinite";
export type NationalDayPricePeriod = "sixmonth" | "annual" | "multiyear" | "lifetime" | `${number}y`;

function normalizeTier(tier: string): "lite" | "pro" | "infinity" {
  return tier === "infinite" ? "infinity" : tier as "lite" | "pro" | "infinity";
}

function normalizeSector(segment: string): keyof typeof NATIONAL_DAY_PLAN_PRICES {
  const value = segment.toLowerCase();
  if (value in NATIONAL_DAY_PLAN_PRICES) return value as keyof typeof NATIONAL_DAY_PLAN_PRICES;
  if (["store", "commerce", "e-commerce"].includes(value)) return "ecommerce";
  return "other";
}

export function multiYearNationalDayBasePrice(annual: number, years: number) {
  let total = 0;
  for (let i = 0; i < years; i++) total += annual * Math.max(1 - i * 0.05, 0.6);
  return Math.round(total);
}

export function getNationalDayBasePrice(
  tier: string,
  period: string,
  segment: string,
  years = Number(period.match(/^(\d+)y$/)?.[1] || 2),
) {
  if (!["lite", "pro", "infinity", "infinite"].includes(tier)) return 0;
  const prices = NATIONAL_DAY_PLAN_PRICES[normalizeSector(segment)][normalizeTier(tier)];
  if (period === "sixmonth") return prices.sm;
  if (period === "annual") return prices.yr;
  if (period === "lifetime") return prices.life;
  if (period === "multiyear" || /^\d+y$/.test(period)) return multiYearNationalDayBasePrice(prices.yr, Math.max(1, years));
  return 0;
}
