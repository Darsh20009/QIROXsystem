...
import {
  ...
  MessageCircle, BarChart2, Bell as BellIcon, RefreshCcw,
...
const ALL_NAV: Record<string, NavItem> = {
  // ── Main Operations ───────────────────────────────────────────────────────
  workspace:         { id: "workspace",         labelAr: "مساحات العمل",        labelEn: "Workspaces",            icon: LayoutDashboard, href: "/employee/workspace",         group: "main" },
  ...
  project_renewals:  { id: "project_renewals",  labelAr: "طلبات تجديد المشاريع", labelEn: "Project Renewals",      icon: RefreshCcw,      href: "/admin/project-renewals",       group: "main" },
  ...
  cs_chat:           { id: "cs_chat",           labelAr: "دردشة العملاء",        labelEn: "Client Chat",          icon: MessageCircle,   href: "/cs-chat",                    group: "main" },
  ...
  admin_whatsapp:    { id: "admin_whatsapp",    labelAr: "ربط واتساب API",      labelEn: "WhatsApp API",          icon: MessageSquare,   href: "/admin/whatsapp",             group: "tools" },
  ...
  store_whatsapp:    { id: "store_whatsapp",    labelAr: "واتساب أعمال للمتاجر",labelEn: "Store WhatsApp",       icon: MessageSquare,   href: "/admin/store-whatsapp",       group: "config" },
...
const MOBILE_ROLE_ITEMS: Record<string, string[]> = {
  admin:         ["dashboard", "orders", "customers", "leads_data", "crm", "whatsapp_crm", "admin_whatsapp", "employees", "finance", "invoices", "receipts", "payroll", "reports", "quotations", "kanban", "attendance", "qmeet", "builder", "deployment", "sector_guide", "qi_agent", "checklist", "settings", "project_renewals", "mail", "profile", "changelog"],
  manager:       ["dashboard", "orders", "customers", "leads_data", "crm", "whatsapp_crm", "admin_whatsapp", "employees", "finance", "invoices", "receipts", "reports", "quotations", "kanban", "attendance", "qmeet", "builder", "deployment", "qi_agent", "checklist", "mail", "profile", "changelog"],
  developer:     ["dashboard", "mod_requests", "orders", "whatsapp_crm", "builder", "deployment", "sector_guide", "qi_agent", "checklist", "qmeet", "kanban", "mail", "profile", "my_finance", "changelog"],
  designer:      ["dashboard", "mod_requests", "whatsapp_crm", "builder", "checklist", "qmeet", "mail", "profile", "my_finance", "changelog"],
  ...
  accountant:    ["dashboard", "whatsapp_crm", "finance", "invoices", "receipts", "payroll", "wallet", "reports", "checklist", "mail", "profile", "my_finance"],
  support:       ["dashboard", "whatsapp_crm", "cs_chat", "support_tickets", "contact_msgs", "customers", "leads_data", "checklist", "mail", "profile", "my_finance"],
  data_entry:    ["dashboard", "whatsapp_crm", "orders", "customers", "partners", "kanban", "employees", "contact_msgs", "mail", "profile", "changelog"],
  hr:            ["dashboard", "whatsapp_crm", "employees", "payroll", "attendance", "checklist", "mail", "profile", "my_finance"],
  merchant:      ["dashboard", "whatsapp_crm", "shipments", "checklist", "mail", "profile", "my_finance"],
  content:       ["dashboard", "whatsapp_crm", "products", "news", "marketing_posts", "checklist", "mail", "profile", "my_finance"],
};

const ROLE_ITEMS: Record<string, string[]> = {
  admin: [
    // Core operations
    "dashboard", "orders", "customers", "leads_data", "crm", "whatsapp_crm", "new_order", "subscriptions", "project_renewals",
    // Client services
    "quotations", "contracts", "consultations", "mod_requests", "cs_chat", "support_tickets", "contact_msgs", "sla",
    // Products & catalog
    ...
    "api_keys", "store_whatsapp", "cron_jobs", "atlas_db", "qirox_settings", "extra_addons", "data_requests", "countries",
    // Personal
  ...
  manager: [
    ...
    "employees", "attendance",
    // Tools
    "kanban", "qmeet", "builder", "deployment", "qi_agent", "ai_hub", "local_ai", "admin_whatsapp", "checklist", "sys_dashboards", "activity_log",
    // Config (limited)
  ...
  accountant:    ["dashboard", "whatsapp_crm", "finance", "invoices", "receipts", "payroll", "wallet", "installments", "profit_report", "sales_reports", "reports", "bank_settings", "checklist", "mail", "profile", "my_finance"],
  support:       ["dashboard", "whatsapp_crm", "cs_chat", "support_tickets", "contact_msgs", "customers", "leads_data", "sla", "checklist", "mail", "profile", "my_finance"],
  data_entry:    ["dashboard", "whatsapp_crm", "orders", "customers", "partners", "kanban", "employees", "contact_msgs", "mail", "profile", "changelog"],
  hr:            ["dashboard", "whatsapp_crm", "employees", "payroll", "attendance", "checklist", "mail", "profile", "my_finance"],
  merchant:      ["dashboard", "whatsapp_crm", "shipments", "checklist", "mail", "profile", "my_finance"],
  content:       ["dashboard", "whatsapp_crm", "products", "news", "marketing_posts", "templates", "checklist", "mail", "profile", "my_finance"],
};
const DEFAULT_ITEMS = ["dashboard", "mail", "profile"];

export function getAllowedEmployeeNavItems(role: string, allowedPages?: string[] | null): NavItem[] {
...
const WORKSPACE_META: Record<string, { labelAr: string; labelEn: string; descriptionAr: string; descriptionEn: string; icon: React.ElementType; candidates: string[] }> = {
  overview: {
    labelAr: "لوحة العمل", labelEn: "Work dashboard",
    descriptionAr: "أهم ما يحتاج انتباهك اليوم في مكان واحد.",
    descriptionEn: "Your most important work for today, in one place.",
    icon: LayoutDashboard, candidates: ["/employee/workspace", "/employee/role-dashboard"],
  },
  clients: {
    labelAr: "العملاء والمبيعات", labelEn: "Clients & sales",
  ...
  delivery: {
    labelAr: "المشاريع والتنفيذ", labelEn: "Projects & delivery",
    descriptionAr: "حالة المشاريع، المهام، وطلبات التعديل.",
    descriptionEn: "Project status, assigned tasks, and change requests.",
    icon: Package, candidates: ["/admin/orders", "/admin/kanban", "/admin/mod-requests", "/employee/checklist"],
  },
  communication: {
    labelAr: "التواصل", labelEn: "Communication",
    descriptionAr: "الرسائل، واتساب، الاجتماعات، ودعم العملاء.",
    descriptionEn: "Mail, WhatsApp, meetings, and customer support.",
    icon: MessageSquare, candidates: ["/employee/mail", "/employee/whatsapp-crm", "/cs-chat", "/admin/qmeet"],
  },
  finance: {
    labelAr: "المالية والتقارير", labelEn: "Finance & reports",
    ...
    icon: BarChart3, candidates: ["/admin/finance", "/admin/analytics", "/admin/sales-reports", "/employee/my-finance"],
  },
  administration: {
    labelAr: "الفريق والإدارة", labelEn: "Team & administration",
    descriptionAr: "الموظفون، الصلاحيات، وإعدادات التشغيل.",
    descriptionEn: "Employees, permissions, and operational settings.",
    icon: Settings, candidates: ["/admin", "/admin/employees", "/admin/settings"],
  },
};
...
function AttendanceStatusPill() {
  ...
  const { data } = useQuery<any>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/attendance/status", { credentials: "include" });
      if (!r.ok) return null;
      return r.json();
    },
  ...
  if (!data) return null;
  const isClockedIn = data.clockedIn || data.isCheckedIn;
  return (
      <div className="mx-2 mb-1 px-3 py-2 rounded-xl flex items-center gap-2 bg-slate-100/80 dark:bg-white/[0.04]">
      <span className={`w-2 h-2 rounded-full flex-shrink-0 ${isClockedIn ? "bg-emerald-500 animate-pulse" : "bg-black/20 dark:bg-white/20"}`} />
      <span className="text-[11px] font-semibold text-black/50 dark:text-white/45 flex-1">
        {isClockedIn ? (L ? "حاضر" : "Clocked In") : (L ? "خارج" : "Clocked Out")}
      </span>
      {isClockedIn && data.checkInTime && (
        <span className="text-[10px] text-black/30 dark:text-white/25" dir="ltr">
...
function DailyHub({ onAiOpen, onItemClick, role }: { onAiOpen: () => void; onItemClick?: () => void; role: string }) {
  ...
  const isSupport = ["support", "admin", "manager"].includes(role);
  const { data: csData } = useQuery<any[]>({
    queryKey: ["/api/cs/sessions", "waiting"],
    queryFn: async () => {
      const r = await fetch("/api/cs/sessions?status=waiting", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: isSupport,
  ...
  const waitingCount = isSupport ? (csData?.length ?? 0) : 0;
  ...
  const quickTools = [
    ...
    ...(isSupport ? [{ icon: MessageCircle, label: "دردشة", href: "/cs-chat", count: waitingCount, color: "text-orange-500" }] : [{ icon: Video, label: "اجتماع", href: "/admin/qmeet", count: undefined as any, color: "text-purple-500" }]),
    { icon: LayoutGrid, label: "أدواتي", href: "/our-tools", color: "text-amber-500" },
  ];

  return (
        <div className="px-3 py-3 border-b border-slate-200/80 dark:border-white/[0.06]">
      {/* Time + AI CTA */}
      <button
        onClick={() => { onAiOpen(); onItemClick?.(); }}
        data-testid="daily-hub-ai"
        className="w-full rounded-xl bg-[#0d263d] p-2.5 flex items-center gap-2.5 hover:bg-[#123653] transition-colors group mb-2.5"
      >
...
function MobileMyTasks() {
  ...
  return (
    <div className="md:hidden mx-3 mb-3 rounded-2xl bg-white dark:bg-gray-900 border border-black/[0.06] dark:border-white/[0.06] shadow-sm overflow-hidden">
...
export default function EmployeeLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = useMemo(() => {
    ...
    return getAllowedEmployeeNavItems(role, customPages);
  ...
  const workspaceItems = useMemo(() => {
    ...
    return Object.entries(WORKSPACE_META)
      .map(([id, meta]) => {
        const item = meta.candidates.map(href => byHref.get(href)).find(Boolean);
        if (!item) return null;
        return { id, meta, item: item as NavItem };
      })
      .filter(Boolean) as { id: string; meta: typeof WORKSPACE_META[string]; item: NavItem }[];
  }, [navItems]);

  const workspaceHrefs = useMemo(() => new Set(workspaceItems.map(({ item }) => item.href)), [workspaceItems]);
  const currentPageIsInAllTools = navItems.some(item => item.href !== "/employee/role-dashboard" && item.href !== "/employee/workspace" && location.startsWith(item.href) && !workspaceHrefs.has(item.href));
  ...
  function SidebarContent({ onItemClick, isMobile }: { onItemClick?: () => void; isMobile?: boolean }) {
    ...
    return (
      <div className="flex flex-col h-full" dir="rtl">
        {/* Brand */}
        <div className="px-4 pt-5 pb-4 border-b border-slate-200/80 dark:border-white/[0.06]">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#0d263d] rounded-xl flex items-center justify-center flex-shrink-0">
              <img src="/qirox-icon-nobg.png" alt="Q" className="w-5 h-5 object-contain" />
            </div>
        ...
        {/* Compact workspace navigation. The full, permission-filtered list stays available below. */}
        <nav className="flex-1 overflow-y-auto py-3 px-2 scrollbar-hide">
          <p className="text-[9px] font-black text-black/25 dark:text-white/22 uppercase tracking-[0.18em] px-2 mb-1.5">
            {L ? "مساحات العمل" : "Workspaces"}
          </p>
          <div className="space-y-0.5">
            {workspaceItems.map(({ id, meta, item }) => {
              const active = isActive(item.href) || (id === "overview" && location === "/employee/workspace");
              return (
                <Link key={id} href={item.href}>
                  <button
                    onClick={onItemClick}
                    data-testid={`employee-workspace-${id}`}
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[13px] font-semibold transition-all duration-150 text-right ${
                      active
                        ? "bg-[#0d263d] text-white shadow-sm"
                        : "text-black/60 dark:text-white/60 hover:bg-black/[0.04] dark:hover:bg-white/[0.05] hover:text-black dark:hover:text-white"
                    }`}
                  >
                    <meta.icon className={`w-4 h-4 flex-shrink-0 ${active ? "" : "opacity-60"}`} />
                    <span className="flex-1 text-right truncate">{L ? meta.labelAr : meta.labelEn}</span>
          ...
          <button
            ...
            data-testid="employee-all-tools-toggle"
            className="w-full flex items-center gap-2.5 px-3 py-2.5 mt-3 rounded-xl text-[12px] font-bold text-black/45 dark:text-white/40 hover:bg-black/[0.04] dark:hover:bg-white/[0.05] hover:text-black dark:hover:text-white transition-colors text-right"
          >
            <Layers className="w-4 h-4 opacity-60" />
            <span className="flex-1">{L ? "كل الأدوات والصفحات" : "All tools & pages"}</span>
            <ChevronDown className={`w-3.5 h-3.5 transition-transform ${allNavigationOpen || currentPageIsInAllTools ? "rotate-180" : ""}`} />
          </button>

          {(allNavigationOpen || currentPageIsInAllTools) && (
            <div className="mt-2 pt-2 border-t border-black/[0.06] dark:border-white/[0.06] space-y-4">
              {itemsOrderedGroups.map(group => (
                <div key={group}>
                  ...
                  <div className="space-y-0.5">
                    {itemsGrouped[group].filter(item => !workspaceHrefs.has(item.href)).map(item => {
                      ...
                      return (
                        <Link key={item.id} href={item.href}>
                          <button
                            ...
                            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[12px] font-semibold transition-all duration-150 text-right ${
                              active
                                 ? "bg-[#0d263d] text-white shadow-sm"
                                : "text-black/55 dark:text-white/55 hover:bg-black/[0.04] dark:hover:bg-white/[0.05] hover:text-black dark:hover:text-white"
                            }`}
                          >
                            <item.icon className={`w-3.5 h-3.5 flex-shrink-0 ${active ? "" : "opacity-60"}`} />
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f6] dark:bg-gray-950 flex" dir="rtl">
      {/* ── Desktop Sidebar ── */}
      <aside className="hidden md:flex w-[272px] flex-col fixed top-0 right-0 bottom-0 bg-[#fbfcfd] dark:bg-gray-900 border-l border-slate-200/80 dark:border-white/[0.06] z-40">
        <SidebarContent />
      </aside>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 right-0 bottom-0 w-72 bg-[#fbfcfd] dark:bg-gray-900 z-50 md:hidden shadow-xl"
            >
      ...
      <div className="flex-1 md:mr-[272px] flex flex-col min-h-screen">
        {/* Mobile top bar */}
        <div className="md:hidden flex items-center justify-between px-4 py-3 bg-[#fbfcfd] dark:bg-gray-900 border-b border-slate-200/80 dark:border-white/[0.06] sticky top-0 z-30">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 bg-[#0d263d] rounded-lg flex items-center justify-center">
              <img src="/qirox-icon-nobg.png" alt="Q" className="w-4 h-4 object-contain" />
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-xl bg-[#0d263d]"
      ...
      {/* ── Mobile Bottom Nav ── */}
      <nav
        className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#fbfcfd] dark:bg-gray-900 border-t border-slate-200/80 dark:border-white/[0.06]"
        dir="rtl"
      >
