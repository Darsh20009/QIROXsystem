...
function ReferralWidget() {
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/referral/my-code", { credentials: "include" });
      if (!r.ok) return null;
      return r.json();
    },
    retry: false,
  ...
  const copyCode = () => { navigator.clipboard.writeText(data.code).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); };
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-3">
      <div className="rounded-2xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/[0.06] p-4 flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-500/10 flex items-center justify-center shrink-0">
          <span className="text-xl">🎁</span>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-black text-amber-900 dark:text-amber-300">أدعُ أصدقاءك واكسب رصيداً</p>
          <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">رصيدك المكتسب: <span className="font-bold">{data.creditsEarned || 0}</span> ر.س — دعوات: <span className="font-bold">{data.referrals?.length || 0}</span></p>
        </div>
        <button
          onClick={copyCode}
          className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-200 dark:bg-amber-500/20 text-amber-900 dark:text-amber-300 text-xs font-black hover:bg-amber-300 dark:hover:bg-amber-500/30 transition-colors"
        >
          {copied ? "✓ تم النسخ" : data.code}
        </button>
      </div>
    </motion.div>
  );
...
function LatestNewsSection() {
  ...
  const news = (data || []).filter((n: any) => n.published !== false).slice(0, 3);
  if (!news.length) return null;

  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mt-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-black text-gray-900 dark:text-white">آخر الأخبار</h2>
        <Link href="/news">
          <span className="text-[11px] text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white transition-colors flex items-center gap-1">
      ...
      <div className="space-y-2">
        {news.map((item: any) => (
          <Link key={item._id || item.id} href="/news">
            <div className="flex items-start gap-3 p-3 bg-white dark:bg-gray-900 rounded-xl border border-black/[0.05] dark:border-white/[0.06] hover:border-black/15 dark:hover:border-white/15 transition-colors group cursor-pointer">
              {item.thumbnail && (
                <img src={item.thumbnail} alt={item.titleAr || item.title} className="w-12 h-12 rounded-lg object-cover flex-shrink-0 bg-gray-100" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-gray-900 dark:text-white truncate group-hover:text-black dark:group-hover:text-white">{item.titleAr || item.title}</p>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 line-clamp-1">{item.excerptAr || item.excerpt || ""}</p>
              </div>
              <ArrowRight className="w-3.5 h-3.5 text-gray-300 dark:text-gray-600 flex-shrink-0 mt-1 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors rotate-180" />
            </div>
          </Link>
        ))}
      </div>
...
export default function ClientDashboardSimple({ user }: Props) {
  ...
  const { data: modQuota } = useQuery<any>({ queryKey: ["/api/mod-quota"] });
  ...
  const requestProjectRenewal = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "تم إرسال طلب التجديد", description: "سيراجع الفريق طلبك ويتواصل معك قريباً" });
    },
    onError: (error: any) => {
      setRenewingProjectId(null);
      toast({ title: "تعذر إرسال طلب التجديد", description: error?.message, variant: "destructive" });
    },
  });
  ...
  const dashboardProjects = (projects as any[]).filter((p: any) =>
    p.status !== "completed" && p.status !== "closed" || p.subscription?.status === "expired",
  );
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f9] dark:bg-gray-950" dir="rtl">

      {/* ── Hero Banner — Video Background ── */}
      <div className="bg-[#0d263d] relative overflow-hidden" style={{ minHeight: 220 }}>
        <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-[#f5f7f9] dark:from-gray-950 to-transparent z-10" />
        ...
        <div className="max-w-2xl mx-auto px-4 pt-8 pb-16 relative z-10">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Link href="/prices">
               <Button size="sm" className="bg-white text-[#0d263d] hover:bg-slate-100 rounded-xl h-10 px-5 font-bold gap-2 border-0" data-testid="button-new-order">
                  <Plus className="w-4 h-4" /> طلب جديد
                </Button>
              </Link>
              <Link href="/cs-chat">
                 <Button size="sm" variant="outline" className="rounded-xl h-10 px-4 border-white/30 text-white/80 bg-white/[0.08] hover:bg-white/[0.14] hover:text-white gap-2">
      ...
      <div className="max-w-2xl mx-auto px-4 pb-20">
        ...
        {(() => {
          ...
          const isLifetimePlan = bestQuota?.planPeriod === "lifetime";
          const hasUnlimited = bestQuota?.hasUnlimitedAddon;
          const quotaUsed = bestQuota?.usedThisPeriod ?? 0;
          const quotaTotal = bestQuota?.modificationsPerPeriod ?? 0;
          const quotaLeft = Math.max(0, quotaTotal - quotaUsed);
          const showMods = modQuota?.hasOrders && bestQuota && !isLifetimePlan && !hasUnlimited;

          const statsItems = [
            { label: "طلباتي", value: (orders as any[]).length, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-500/10", icon: FileText },
            { label: "مشاريع نشطة", value: activeProjects.length, color: "text-[#0d5575]", bg: "bg-[#e8f1f5] dark:bg-cyan-500/10", icon: Activity },
          ...
          return (
            <>
              ...
              {/* ── Modifications Quota Card ── */}
              {showMods && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <div className={`mt-3 rounded-2xl border px-4 py-3 flex items-center gap-3 ${
                    quotaLeft === 0
                      ? "bg-red-50 dark:bg-red-900/15 border-red-200 dark:border-red-500/20"
                      : quotaLeft <= 2
                      ? "bg-orange-50 dark:bg-orange-900/15 border-orange-200 dark:border-orange-500/20"
                      : "bg-white dark:bg-gray-900 border-black/[0.06] dark:border-white/[0.07]"
                  }`}>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      quotaLeft === 0 ? "bg-red-100 dark:bg-red-900/30" :
                      quotaLeft <= 2 ? "bg-orange-100 dark:bg-orange-900/30" : "bg-black/[0.04] dark:bg-white/[0.06]"
                    }`}>
                      <Wrench className={`w-5 h-5 ${quotaLeft === 0 ? "text-red-500" : quotaLeft <= 2 ? "text-orange-500" : "text-gray-500"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <p className="text-sm font-bold text-gray-900 dark:text-white">تعديلات المشروع</p>
                        <span className={`text-sm font-black ${quotaLeft === 0 ? "text-red-600" : quotaLeft <= 2 ? "text-orange-600" : "text-gray-900 dark:text-white"}`}>
                          {quotaLeft} / {quotaTotal}
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-black/[0.06] dark:bg-white/[0.08] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${quotaLeft === 0 ? "bg-red-400" : quotaLeft <= 2 ? "bg-orange-400" : "bg-green-400"}`}
                          style={{ width: `${quotaTotal > 0 ? (quotaLeft / quotaTotal) * 100 : 0}%` }} />
                      </div>
                      <p className="text-[10px] mt-1 text-gray-400 dark:text-slate-500">
                        {quotaLeft === 0
                           ? "استنفدت رصيد التعديلات — اطلب إضافة رصيد"
                          : quotaLeft <= 2
                           ? `متبقي ${quotaLeft} تعديل فقط هذه الفترة`
                          : `متبقي ${quotaLeft} تعديل من أصل ${quotaTotal} هذه الفترة`}
                      </p>
                    </div>
                    {quotaLeft === 0 && (
                      <Link href="/client/mod-requests">
                        <Button size="sm" className="h-8 px-3 text-xs bg-red-500 hover:bg-red-600 text-white rounded-xl shrink-0">
                          إضافة
                        </Button>
                      </Link>
                    )}
              ...
              {modQuota?.hasOrders && bestQuota?.hasUnlimitedAddon && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <div className="mt-3 rounded-2xl border bg-green-50 dark:bg-green-900/15 border-green-200 dark:border-green-500/20 px-4 py-2.5 flex items-center gap-3">
                    <Sparkles className="w-4 h-4 text-green-500 flex-shrink-0" />
                    <p className="text-xs font-bold text-green-700 dark:text-green-300">تعديلات غير محدودة — باقة مميزة ✨</p>
                  </div>
                </motion.div>
              )}
            </>
        ...
        <div className="mt-6">
              <div className="flex bg-slate-100 dark:bg-white/[0.05] rounded-xl p-1 mb-4">
            <button
              onClick={() => setActiveTab("projects")}
               className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "projects" ? "bg-white dark:bg-gray-800 border border-slate-200 dark:border-white/10 text-gray-900 dark:text-white" : "text-gray-400 dark:text-slate-500"}`}
            ...
            <button
              onClick={() => setActiveTab("orders")}
               className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "orders" ? "bg-white dark:bg-gray-800 border border-slate-200 dark:border-white/10 text-gray-900 dark:text-white" : "text-gray-400 dark:text-slate-500"}`}
                          ...
                          <div className="p-5">
                            ...
                            <div className="mb-4">
                              <ProjectSubscriptionCard
                                ...
                                lang="ar"
                                compact
                                onRequestRenewal={() => requestProjectRenewal.mutate(String(project.id || project._id))}
                                renewalPending={renewingProjectId === String(project.id || project._id)}
                              />
                            </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          {[
            { label: "تواصل مع الدعم",  desc: "نرد خلال دقائق",   href: "/cs-chat",         icon: Headphones, color: "text-blue-600",   bg: "bg-blue-50 dark:bg-blue-500/[0.08]",   border: "border-blue-100 dark:border-blue-500/20" },
             { label: "طلب تعديل",        desc: "على مشروعك الحالي", href: "/support",          icon: RefreshCw,  color: "text-[#0d5575]", bg: "bg-[#e8f1f5] dark:bg-cyan-500/[0.08]", border: "border-[#c9e1e9] dark:border-cyan-500/20" },
