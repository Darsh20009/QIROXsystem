...
export default function AdminJobs() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => toast({ title: L ? "فشل حذف الوظيفة" : "Delete failed", variant: "destructive" }),
  });
  ...
  const updateAppMutation = useMutation({
    ...
    onError: () => toast({ title: L ? "فشل تحديث الطلب" : "Update failed", variant: "destructive" }),
  });
  ...
  const handleEdit = (job: Job) => {
    ...
    setFormData({
      ...
      status: job.status,
      deadline: (job as any).deadline ? new Date((job as any).deadline).toISOString().split("T")[0] : "",
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-black/[0.06] dark:border-white/[0.06]">
                      {[L ? "الوظيفة" : "Job", L ? "النوع" : "Type", L ? "الراتب" : "Salary", L ? "آخر موعد" : "Deadline", L ? "الطلبات" : "Applications", L ? "الحالة" : "Status", ""].map(h => (
                    ...
                    {jobs?.map((job) => {
                      ...
                      return (
                        <tr key={job.id} className="border-b border-black/[0.03] dark:border-white/[0.03] hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors" data-testid={`row-job-${job.id}`}>
                          ...
                          <td className="p-4 text-xs text-black/50 dark:text-white/40">
                            {(job as any).deadline
                              ? (() => {
                                  const d = new Date((job as any).deadline);
                                  const past = d < new Date();
                                  return <span className={past ? "text-red-500 font-semibold" : ""}>{d.toLocaleDateString(L ? "ar-SA" : "en-US", { year: "numeric", month: "short", day: "numeric" })}</span>;
                                })()
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) { setOpen(false); setEditingId(null); } }}>
        <DialogContent className="bg-white dark:bg-gray-900 border-black/[0.06] dark:border-white/[0.06] max-w-2xl max-h-[90vh] overflow-y-auto" dir={dir}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div>
              <label className="block text-sm font-medium text-black/60 dark:text-white/60 mb-1.5">{L ? "آخر موعد للتقديم" : "Application Deadline"}</label>
              <Input type="date" value={formData.deadline} onChange={(e) => setFormData(f => ({ ...f, deadline: e.target.value }))} data-testid="input-job-deadline" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
