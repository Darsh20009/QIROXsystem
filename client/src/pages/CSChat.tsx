...
function VoicePlayer({ url, isMe = false }: { url: string; isMe?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-[150px]">
      ...
      <button onClick={() => { if (!ref.current) return; if (playing) { ref.current.pause(); setPlaying(false); } else { ref.current.play().then(() => setPlaying(true)).catch(e => { console.error("[VoicePlayer] play failed:", e); setPlaying(false); }); return; } setPlaying(false); }} className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${btnCls}`}>
...
function ClientView({ user }: { user: any }) {
  ...
  const closeMutation = useMutation({
    ...
    onError: () => toast({ title: L ? "فشل إغلاق المحادثة" : "Close failed", variant: "destructive" }),
  });
  ...
  const rateMutation = useMutation({
    ...
    onError: () => toast({ title: L ? "فشل إرسال التقييم" : "Rating failed", variant: "destructive" }),
  });
