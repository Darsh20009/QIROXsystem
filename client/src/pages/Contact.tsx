...
export default function Contact() {
  ...
  const { data: publicSettings } = useQuery<{ whatsapp?: string; contactEmail?: string }>({
    queryKey: ["/api/public/settings"],
    staleTime: 10 * 60 * 1000,
  });
  const supportEmail = publicSettings?.contactEmail || "info@qiroxstudio.online";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
            <motion.div initial="hidden" animate="visible">
              <motion.div variants={fadeUp} custom={0} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/[0.1] bg-white/[0.05] mb-6">
                ...
                <span className="text-white/45 text-xs tracking-wider uppercase">{T.badge}</span>
              </motion.div>
              <motion.h1 variants={fadeUp} custom={1} className="text-4xl md:text-5xl lg:text-6xl font-black font-heading text-white mb-5 tracking-tight leading-[1.08]">
                {T.title} <span className="text-white/30">{T.titleHighlight}</span>
              </motion.h1>
              <motion.p variants={fadeUp} custom={2} className="text-white/40 text-base max-w-sm leading-relaxed">
                {T.subtitle}
              </motion.p>
            </motion.div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <motion.div initial="hidden" animate="visible" className="md:col-span-2 space-y-4">
            <div className="bg-white dark:bg-gray-900/60 border border-black/[0.06] dark:border-white/[0.06] rounded-2xl p-7 shadow-sm">
              ...
              <p className="text-sm text-black/40 dark:text-white/40">{supportEmail}</p>
            </div>
