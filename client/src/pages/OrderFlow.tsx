...
export default function OrderFlow() {
  ...
  // Auto-skip step 1 (package selection) when arriving from Prices page with all params set
  const hasPreselectedPlan = !!(planFromUrl && priceFromUrl > 0 && segmentFromUrl);
  ...
  const resendMutation = useMutation({
    ...
    onError: () => toast({ title: "فشل إعادة إرسال الرمز", variant: "destructive" }),
  });
  useEffect(() => {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (data) => {
      setSubmittedOrder({ id: data.orderNumber || data.id || data._id, amount: grandTotal, walletUsed: data.walletUsed || 0 });
      toast({ title: "✅ تم إرسال طلبك بنجاح!" });
      // Auto-redirect to dashboard after 8 seconds if no bank transfer needed
      if (fullyPaidByWallet) {
        setTimeout(() => setLocation("/dashboard"), 8000);
      }
    },
    ...
    <div className="min-h-screen bg-[#fafafa] dark:bg-gray-950" dir={dir}>
      ...
      <div className="max-w-xl mx-auto pt-20 px-4 pb-24">
        ...
        <div className="mt-6 flex flex-col gap-3 justify-center">
          <Button className="gap-2 rounded-xl h-12 bg-black dark:bg-white text-white dark:text-black hover:opacity-80 font-black text-base" onClick={() => setLocation("/dashboard")} data-testid="button-go-dashboard">
            <ArrowLeft className="w-5 h-5" /> متابعة مشروعي ومتابعة حصص التعديل
          </Button>
          <p className="text-center text-xs text-black/35 dark:text-white/35">سيتم توجيهك تلقائياً إلى لوحة التحكم خلال 8 ثوانٍ</p>
        </div>
      </div>
      <Footer />
    </div>
                            ...
                            {planFromUrl === tier && priceFromUrl > 0 && (
                              <div className={`inline-flex items-baseline gap-1.5 mb-3 px-3 py-1.5 rounded-xl ${v.isDark ? "bg-white/10" : "bg-black/[0.04] dark:bg-white/[0.04]"}`}>
                                {currency.isSaudi ? (
                                  ...
                                  <>
                                    <span className={`text-2xl font-black ${v.isDark ? "text-white" : "text-black dark:text-white"}`}>{currency.formatRaw(currency.convert(priceFromUrl))}</span>
                                    <span className={`text-sm ${v.isDark ? "text-white/70" : "text-black/60 dark:text-white/60"}`}>{currency.symbolShort}</span>
                                    <span className={`text-xs ${v.isDark ? "text-white/40" : "text-black/30 dark:text-white/30"}`}>≈ {priceFromUrl.toLocaleString()} ر.س</span>
                                  </>
                                )}
                              </div>
                            )}
        ...
        <DialogContent className="sm:max-w-sm rounded-3xl" dir={dir}>
          ...
          <div className="flex flex-col gap-3 mt-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 h-12 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-gray-900 hover:bg-black/[0.03] dark:hover:bg-white/[0.05] transition-all font-bold text-sm text-black dark:text-white"
            >
            ...
            <div className="relative flex items-center gap-3">
              ...
              <span className="text-xs text-black/30 dark:text-white/30">{lang === "ar" ? "أو" : "or"}</span>
              <div className="flex-1 h-px bg-black/[0.07] dark:bg-white/[0.07]" />
            </div>
            <Button
              onClick={() => {
                try { sessionStorage.setItem("qirox_order_return", "true"); } catch {}
                setShowAuthModal(false);
                setLocation("/register?redirect=/order");
              }}
              variant="outline"
              className="w-full h-11 rounded-2xl font-bold border-black/12 dark:border-white/12 text-sm"
            >
