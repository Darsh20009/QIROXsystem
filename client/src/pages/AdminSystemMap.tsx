...
import {
  ...
  Globe, Shield, ShieldCheck, Newspaper, Briefcase, ChevronRight, RefreshCw,
    ...
    <Link href={href}>
      <motion.div
        ...
        className={`relative p-3.5 rounded-2xl border cursor-pointer transition-all duration-200 bg-white dark:bg-[#161b22]
          ${urgent && subCount ? "border-black/10 dark:border-white/10 shadow-amber-50 shadow-md" : "border-black/[0.07] dark:border-white/[0.07] hover:border-black/20 dark:hover:border-white/20 hover:shadow-sm"}`}
      ...
      >
        <div className="flex items-start gap-2.5">
          ...
          {urgent && subCount && subCount > 0 ? (
            <span className="text-[10px] font-black bg-black dark:bg-white text-white rounded-full min-w-[20px] h-5 px-1.5 flex items-center justify-center shrink-0 mt-0.5">
              {subCount}
            </span>
          ) : subCount && subCount > 0 ? (
    ...
    <div className={`rounded-2xl border-2 ${borderColor} p-4 bg-white/60 dark:bg-white/[0.04] backdrop-blur-sm`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        {badge !== undefined && badge > 0 && (
          <span className="mr-auto text-[9px] font-black bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white rounded-full px-2 py-0.5">
            {badge} بانتظار
          </span>
        )}
      </div>
...
const orderStatusMap: any = {
  pending: { label: "انتظار", color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06] border-black/10 dark:border-white/10" },
  approved: { label: "موافق", color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06] border-black/10 dark:border-white/10" },
  in_progress: { label: "تنفيذ", color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06] border-black/10 dark:border-white/10" },
  completed: { label: "مكتمل", color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06] border-black/10 dark:border-white/10" },
  rejected: { label: "مرفوض", color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06] border-black/10 dark:border-white/10" },
    ...
    <div dir={dir} className="min-h-screen bg-[#f7f7f7] dark:bg-[#0d1117]">
      ...
      <div className="bg-white dark:bg-[#161b22] border-b border-black/[0.07] dark:border-white/[0.07] px-6 py-4 sticky top-0 z-20 backdrop-blur-md bg-white/95 dark:bg-[#161b22]/95">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {totalPending > 0 && (
              <div className="flex items-center gap-1.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 rounded-xl px-3 py-1.5">
                <AlertTriangle className="w-3.5 h-3.5 text-black dark:text-white" />
                <span className="text-[11px] font-bold text-black dark:text-white">{totalPending} يحتاج انتباه</span>
              </div>
            )}
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-5 space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              { label: "إجمالي الإيرادات", value: `${((ov.totalRevenue || 0) / 1000).toFixed(0)}K`, unit: "SAR_ICON", icon: DollarSign, color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06]", sub: "من الطلبات المكتملة" },
              { label: "إجمالي الطلبات", value: ov.totalOrders || 0, icon: ShoppingBag, color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06]", sub: `${ov.pendingOrders || 0} بانتظار` },
              { label: "العملاء", value: ov.totalClients || 0, icon: Users, color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06]", sub: "عميل مسجّل" },
              ...
              { label: "الفواتير غير المسددة", value: ov.unpaidInvoices || 0, icon: FileText, color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06]", sub: "تحتاج متابعة" },
              { label: "الإشعارات المعلقة", value: ov.pendingNotifications || 0, icon: Bell, color: "text-black dark:text-white bg-black/[0.04] dark:bg-white/[0.06]", sub: "لم تُقرأ بعد" },
            ].map((s, i) => (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <Cluster title="دورة حياة العميل" color="bg-black dark:bg-white" borderColor="border-black/10 dark:border-white/10"
              ...
              <ModCard icon={Users} label="العملاء" count={ov.totalClients} href="/admin/customers"
                color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              ...
              <ModCard icon={ShoppingBag} label="الطلبات" count={ov.totalOrders} subCount={ov.pendingOrders}
                ...
                href="/admin/orders" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
              <FlowArrow label="يولد" />
              <ModCard icon={Wrench} label="طلبات التعديل" count={ov.totalModRequests} subCount={ov.pendingModRequests}
                href="/admin/mod-requests" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
              <FlowArrow label="يُفوتر" />
              <ModCard icon={FileText} label="الفواتير" count={ov.totalInvoices} subCount={ov.unpaidInvoices}
                sub="من إجمالي الفواتير" href="/admin/invoices" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
              <FlowArrow label="يُسدَّد" />
              <ModCard icon={Receipt} label="سندات القبض" count={ov.totalReceipts}
                href="/admin/receipts" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              <FlowArrow label="يُضاف" />
              <ModCard icon={Wallet} label="محافظ العملاء" count={ov.totalTransactions}
                sub="إجمالي حركات المحفظة" href="/admin/wallet" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              ...
              <ModCard icon={Banknote} label="التقسيط" count={ov.totalInstallApps} subCount={ov.pendingInstallApps}
                ...
                href="/admin/installments" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
            </Cluster>
            ...
            <Cluster title="الفريق والموارد البشرية" color="bg-slate-500" borderColor="border-slate-100"
              ...
              <ModCard icon={Activity} label="الحضور اليومي" count={ov.totalAttendanceToday}
                sub="حضروا اليوم" href="/admin/attendance" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              ...
              <ModCard icon={DollarSign} label="كشف الرواتب" count={ov.totalPayroll}
                ...
                href="/admin/payroll" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              <FlowArrow label="ترقيات" />
              <ModCard icon={Shield} label="الترقيات والأدوار" href="/admin/promotions"
                color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              <ModCard icon={ShieldCheck} label="إدارة الصلاحيات" href="/admin/roles"
              ...
              <div className="mt-2 pt-2 border-t border-black/[0.05] dark:border-white/[0.05]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <ModCard icon={Briefcase} label="الوظائف" count={ov.activeJobs}
                    ...
                    href="/admin/jobs" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
                  <ModCard icon={TrendingUp} label="المستثمرون" count={ov.totalInvestors}
                    href="/admin/investors" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
                </div>
              </div>
            </Cluster>

            {/* ── Cluster 3: الخدمات والعمليات ───────────────── */}
            <Cluster title="الخدمات والعمليات" color="bg-black dark:bg-white" borderColor="border-black/10 dark:border-white/10"
              ...
              <ModCard icon={Tag} label="كودات الخصم" count={ov.totalDiscountCodes}
                href="/admin/discount-codes" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              <FlowArrow label="استشارات" />
              <ModCard icon={CalendarCheck} label="الاستشارات" count={ov.totalConsultations}
                href="/admin/consultations" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              <div className="mt-2 pt-2 border-t border-black/[0.05] dark:border-white/[0.05]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <ModCard icon={Newspaper} label="الأخبار" count={ov.publishedNews}
                    ...
                    href="/admin/news" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
            ...
            <Cluster title="التواصل والدعم" color="bg-black dark:bg-white" borderColor="border-black/10 dark:border-white/10"
              ...
              <ModCard icon={MessageSquare} label="صندوق الرسائل" count={ov.totalMessages} subCount={ov.unreadMessages}
                sub="رسالة غير مقروءة" href="/inbox" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
              <FlowArrow />
              ...
              <ModCard icon={LifeBuoy} label="تذاكر الدعم" count={ov.totalSupportTickets} subCount={ov.openSupportTickets}
                sub="تذاكر مفتوحة" href="/admin/support-tickets" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" urgent />
              <FlowArrow />
            ...
            <Cluster title="المالية والتحليلات" color="bg-black dark:bg-white" borderColor="border-black/10 dark:border-white/10"
              icon={DollarSign}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <ModCard icon={FileText} label="الفواتير" count={ov.totalInvoices}
                  href="/admin/invoices" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              ...
              <div className="grid grid-cols-2 gap-2">
                <ModCard icon={Wallet} label="المحافظ" count={ov.totalTransactions}
                  href="/admin/wallet" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
                <ModCard icon={Banknote} label="التقسيط" count={ov.totalInstallApps}
                  href="/admin/installments" color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" />
              </div>
              <FlowArrow />
              <div className="grid grid-cols-2 gap-2">
            ...
            <Cluster title="إعدادات النظام" color="bg-gray-500" borderColor="border-gray-100"
              ...
              <FlowArrow />
              <ModCard icon={Smartphone} label="نشر التطبيق" href="/admin/app-publish"
                color="text-black dark:text-white" bg="bg-black/[0.04] dark:bg-white/[0.06]" sub="Android · iOS · Windows · HarmonyOS" />
              <div className="mt-2 pt-2 border-t border-black/[0.05] dark:border-white/[0.05]">
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <Link key={order._id} href={`/admin/orders`}>
                      <div className="flex items-center gap-3 px-5 py-3 hover:bg-black/[0.01] dark:hover:bg-white/[0.03] transition-colors cursor-pointer" data-testid={`order-row-${order._id}`}>
                        <div className="w-8 h-8 rounded-xl bg-black/[0.04] dark:bg-white/[0.06] flex items-center justify-center flex-shrink-0">
                          <ShoppingBag className="w-3.5 h-3.5 text-black dark:text-white" />
                        </div>
                        <div className="flex-1 min-w-0">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-black/[0.07] dark:border-white/[0.07] p-4">
                ...
                <div className="space-y-2">
                  {[
                    { label: "طلبات معلقة", count: ov.pendingOrders, href: "/admin/orders", icon: ShoppingBag, color: "text-black dark:text-white" },
                    { label: "تذاكر مفتوحة", count: ov.openSupportTickets, href: "/admin/support-tickets", icon: LifeBuoy, color: "text-black dark:text-white" },
                    { label: "تعديلات معلقة", count: ov.pendingModRequests, href: "/admin/mod-requests", icon: Wrench, color: "text-black dark:text-white" },
                    { label: "أقساط معلقة", count: ov.pendingInstallApps, href: "/admin/installments", icon: Banknote, color: "text-black dark:text-white" },
                    { label: "شحنات معلقة", count: ov.pendingShipments, href: "/admin/shipments", icon: Truck, color: "text-slate-600" },
                    { label: "فواتير غير مسددة", count: ov.unpaidInvoices, href: "/admin/invoices", icon: FileText, color: "text-black dark:text-white" },
                  ].filter(i => (i.count || 0) > 0).map(item => (
                    <Link key={item.href} href={item.href}>
                      <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-black/[0.04] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 hover:bg-black/[0.04] dark:bg-white/[0.06] transition-colors cursor-pointer">
                        <div className="flex items-center gap-2">
                          <item.icon className={`w-3.5 h-3.5 ${item.color}`} />
                          <span className="text-[11px] font-bold text-black/70 dark:text-white/70">{item.label}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[11px] font-black text-black dark:text-white">{item.count}</span>
                  ...
                  {totalPending === 0 && (
                    <div className="flex items-center gap-2 py-3 justify-center">
                      ...
                      <p className="text-xs text-black dark:text-white font-bold">كل شيء على ما يرام!</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-black/[0.07] dark:border-white/[0.07] overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-black/[0.05] dark:border-white/[0.05]">
                  <div className="flex items-center gap-2">
                    <LifeBuoy className="w-3.5 h-3.5 text-black dark:text-white" />
                  ...
                  {recentTickets.length === 0 ? (
                    ...
                    <Link key={ticket._id} href="/admin/support-tickets">
                      <div className="flex items-start gap-2.5 px-4 py-3 hover:bg-black/[0.01] dark:hover:bg-white/[0.03] cursor-pointer transition-colors">
                        <div className="w-7 h-7 rounded-xl bg-black/[0.04] dark:bg-white/[0.06] flex items-center justify-center flex-shrink-0 mt-0.5">
                          <LifeBuoy className="w-3 h-3 text-black dark:text-white" />
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 ${ticket.status === "open" ? "bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white border border-black/10 dark:border-white/10" : "bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white border border-black/10 dark:border-white/10"}`}>
