...
import { Loader2, Plus, Edit2, Trash2, Newspaper, Eye, EyeOff, Bell } from "lucide-react";
...
const NEWS_CATEGORIES = ["تقني", "أعمال", "تحديثات", "مشاريع", "إطلاقات", "فريق", "تعليمي", "Other"];
...
export default function AdminNews() {
  ...
  const handleEdit = (item: News) => {
    ...
    setFormData({
      title: item.title,
      titleAr: (item as any).titleAr || "",
      content: item.content,
      excerpt: item.excerpt || "",
      excerptAr: (item as any).excerptAr || "",
                      ...
                      <div className="flex gap-1">
                        ...
                        {item.status === "published" && (
                          <Button
                            ...
                            title={L ? "إشعار المشتركين" : "Notify Subscribers"}
                            onClick={() => {
                              fetch("/api/admin/notifications/broadcast", {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                credentials: "include",
                                body: JSON.stringify({ title: L ? "📰 مقال جديد" : "📰 New Article", body: item.title, link: "/news", target: "all" }),
                              }).then(() => toast({ title: L ? "تم إرسال الإشعار للمشتركين" : "Notification sent to subscribers" }));
                            }}
                            data-testid={`button-notify-news-${item.id}`}
                          >
        ...
        <DialogContent className="bg-white border-black/[0.06] text-black max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-black/60 mb-1.5">العنوان (AR) *</label>
                <Input value={formData.titleAr} onChange={(e) => setFormData(f => ({ ...f, titleAr: e.target.value }))} placeholder="عنوان المقال بالعربي" dir="rtl" data-testid="input-news-title-ar" />
              </div>
              <div>
                <label className="block text-sm font-medium text-black/60 mb-1.5">Title (EN) *</label>
                <Input value={formData.title} onChange={(e) => setFormData(f => ({ ...f, title: e.target.value }))} placeholder="Article title in English" dir="ltr" data-testid="input-news-title" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-black/60 mb-1.5">التصنيف</label>
                <Select value={formData.category} onValueChange={(v) => setFormData(f => ({ ...f, category: v }))}>
                  <SelectTrigger><SelectValue placeholder={L ? "اختر تصنيفاً" : "Choose category"} /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="">{L ? "بدون تصنيف" : "No category"}</SelectItem>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-black/60 mb-1.5">المقتطف (AR)</label>
                <Textarea value={formData.excerptAr} onChange={(e) => setFormData(f => ({ ...f, excerptAr: e.target.value }))} placeholder="وصف مختصر بالعربي..." rows={2} dir="rtl" />
              </div>
              <div>
                <label className="block text-sm font-medium text-black/60 mb-1.5">Excerpt (EN)</label>
                <Textarea value={formData.excerpt} onChange={(e) => setFormData(f => ({ ...f, excerpt: e.target.value }))} placeholder="Short description in English..." rows={2} dir="ltr" data-testid="input-news-excerpt" />
              </div>
            </div>
            <div>
