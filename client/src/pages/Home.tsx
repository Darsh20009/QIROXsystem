import { useState, useMemo, useRef, useEffect, useCallback, type ElementType } from "react";
...
const HOME_TIER_FEATURES: Record<HomePriceTier, { ar: string[]; en: string[] }> = {
  lite: {
    ar: ["موقع احترافي متكامل", "لوحة تحكم سهلة", "نطاق + SSL مجاني", "دعم فني مستمر", "تقارير شهرية"],
    en: ["Professional website", "Easy control panel", "Free domain + SSL", "Ongoing technical support", "Monthly reports"],
  },
  pro: {
    ar: ["كل ميزات لايت ✦", "تطبيق جوال iOS/Android", "بوابة دفع إلكترونية", "إشعارات فورية", "تقارير ذكاء اصطناعي"],
    en: ["Everything in Lite ✦", "iOS/Android mobile app", "Online payment gateway", "Instant notifications", "AI-powered reports"],
  },
  infinity: {
    ar: ["كل ميزات برو ✦✦", "تطوير مخصص بلا قيود", "خادم مستقل مخصص", "مساعد ذكاء اصطناعي", "دعم أولوية 24/7"],
    en: ["Everything in Pro ✦✦", "Unlimited custom development", "Dedicated server", "AI assistant", "Priority 24/7 support"],
  },
};

const HOME_SECTORS_PRICING = [
  { key: "restaurant" as HomePriceSector, icon: UtensilsCrossed, ar: "مطاعم و مقاهي",       en: "Restaurants & Cafés",  color: "from-orange-500 to-amber-500" },
  { key: "ecommerce"  as HomePriceSector, icon: ShoppingBag,     ar: "متاجر",               en: "E-Commerce",           color: "from-blue-500 to-cyan-500" },
  { key: "corporate"  as HomePriceSector, icon: Building2,       ar: "شركات ووكالات",        en: "Companies & Agencies", color: "from-slate-600 to-zinc-700" },
  { key: "healthcare" as HomePriceSector, icon: Heart,           ar: "الصحة والتجميل",       en: "Health & Beauty",      color: "from-rose-500 to-pink-600" },
  { key: "ai"         as HomePriceSector, icon: Bot,             ar: "الذكاء الاصطناعي",     en: "AI Solutions",         color: "from-violet-600 to-purple-700" },
  { key: "education"  as HomePriceSector, icon: GraduationCap,   ar: "التعليم والمنصات",    en: "Education Platforms",  color: "from-indigo-500 to-blue-600" },
];
...
export default function Home() {
  ...
  // Native OAuth: use SFSafariViewController (Capacitor) instead of external Safari
  const openOAuthNative = useCallback(async (path: string) => {
    const { Browser } = await import("@capacitor/browser");
    const { App } = await import("@capacitor/app");
    const separator = path.includes("?") ? "&" : "?";
    const url = `${getServerUrl()}${path}${separator}native=1`;
    const handle = await App.addListener("appUrlOpen", async (data: { url: string }) => {
      await handle.remove();
      try { await Browser.close(); } catch {}
      const callbackUrl = new URL(data.url);
      const token = callbackUrl.searchParams.get("googleToken")
        || callbackUrl.searchParams.get("appleToken");
      const twoFactorToken = callbackUrl.searchParams.get("twoFactorToken");
      if (twoFactorToken) {
        window.location.href = `/login?twoFactorToken=${encodeURIComponent(twoFactorToken)}`;
      } else if (token) {
        saveDeviceToken(token);
        window.location.href = callbackUrl.searchParams.get("next") || "/dashboard";
      } else {
        window.location.href = "/login?error=oauth_failed";
      }
    });
    const fb = await Browser.addListener("browserFinished", async () => {
      await fb.remove(); await handle.remove();
      const r = await fetch(`${getServerUrl()}/api/user`, { credentials: "include" });
  ...
  const { data: customerCountData } = useQuery<{ count: number }>({ queryKey: ["/api/public/customer-count"] });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-black dark:bg-gray-950 dark:text-white w-full overflow-x-hidden" dir={dir}>
      <Navigation />
      ...
      <main className="flex-1">
        ...
        <section className="relative bg-[#f0f0ee] dark:bg-[#0d0d0d] overflow-x-hidden">
          <div className="container mx-auto px-6 md:px-10 max-w-7xl">
            <div className="grid grid-cols-1 md:grid-cols-2 items-end gap-6 pt-24 md:pt-32">
              ...
              <motion.div {...fade(0)} className="flex flex-col justify-center order-1">
                ...
                <h1 className="text-[2.6rem] sm:text-[3.5rem] md:text-[4.25rem] xl:text-[4.75rem] font-black leading-[1.0] tracking-tight text-black dark:text-white mb-6">
                ...
                {!user && (googleEnabled || appleEnabled) && (
                  <motion.div {...fade(0.5)} className="flex flex-col gap-3">
                    ...
                    <div className="flex gap-2">
                      {googleEnabled && (
                        <button onClick={() => isCapacitorNative() ? openOAuthNative("/api/auth/google") : (window.location.href = "/api/auth/google")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-white/[0.04] hover:bg-white dark:hover:bg-white/[0.08] transition-colors text-sm font-bold text-black dark:text-white shadow-sm" data-testid="btn-hero-google-login">
                          <SiGoogle className="w-4 h-4 text-[#4285F4]" />Google
                        </button>
                      )}
                      {appleEnabled && (
                        <button onClick={() => isCapacitorNative() ? openOAuthNative("/api/auth/apple") : (window.location.href = "/api/auth/apple")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-black hover:bg-white dark:hover:bg-white/[0.06] transition-colors text-sm font-bold text-black dark:text-white shadow-sm" data-testid="btn-hero-apple-login">
        ...
        <section id="tab-systems" className="pt-14 pb-0 md:pt-20 bg-white dark:bg-[#0a0a0a]" style={{ overflowX: "clip" }}>
          ...
          <div className="block md:hidden px-5 mb-6">
          ...
          <div className="flex items-start" dir={dir}>
            ...
            <motion.div {...fade(0)} className="hidden md:flex flex-shrink-0 flex-col justify-center w-[260px] px-12 pt-4">
        ...
        <section id="tab-pricing" className="pt-16 pb-24 md:pt-20 md:pb-28 bg-black/[0.02] dark:bg-white/[0.02]">
          <div className="container mx-auto px-5 md:px-8 max-w-6xl">
            ...
            <motion.div {...fade(1)} className="mb-6">
              <p className="text-center text-[10px] font-black tracking-widest uppercase text-black/25 dark:text-white/25 mb-3">
                {ar ? "اختر قطاعك" : "Choose your sector"}
              </p>
            ...
            <motion.div {...fade(2)} className="flex flex-col items-center mb-8 gap-3">
              <div className="flex gap-1 p-1 bg-black/[0.04] dark:bg-white/[0.04] rounded-xl">
                {([
                  { key: "sixmonth" as HomePricePeriod, ar: "6 أشهر",    en: "6 months",  icon: CalendarRange },
                  { key: "annual"   as HomePricePeriod, ar: "سنة",      en: "Annual",    icon: CalendarDays  },
                  { key: "multiyear"as HomePricePeriod, ar: "سنوات+",   en: "Multi-year", icon: CalendarDays  },
                  { key: "lifetime" as HomePricePeriod, ar: "مدى الحياة", en: "Lifetime", icon: Infinity      },
                  ...
                  >
                    <Icon className="w-3.5 h-3.5" />
                    {ar ? label : (key === "sixmonth" ? "6 months" : key === "annual" ? "Annual" : key === "multiyear" ? "Multi-year" : "Lifetime")}
                  </button>
                ))}
              </div>
              ...
              <AnimatePresence>
                {pricingPeriod === "multiyear" && (
                  ...
                  >
                    <span className="text-xs font-bold text-blue-700 dark:text-blue-300">{ar ? "عدد السنوات:" : "Number of years:"}</span>
                    ...
                    <span className="text-xs font-bold text-blue-500 dark:text-blue-400">
                      {ar ? `خصم ${homeMYDiscount(pricingYears)}%` : `${homeMYDiscount(pricingYears)}% discount`}
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto items-stretch">
              {(["lite", "pro", "infinity"] as HomePriceTier[]).map((tier, i) => {
                ...
                if (pricingPeriod === "sixmonth")  { price = prices.sm;                               periodLabel = ar ? "كل 6 أشهر" : "Every 6 months"; }
                else if (pricingPeriod === "annual")    { price = prices.yr;                               periodLabel = ar ? "سنوياً" : "Annually"; }
                else if (pricingPeriod === "multiyear") { price = homeMYPrice(prices.yr, pricingYears);   periodLabel = ar ? `${pricingYears} سنوات` : `${pricingYears} years`; }
                else                                    { price = prices.life;                            periodLabel = ar ? "مدى الحياة" : "Lifetime"; }
                ...
                const tierNames = {
                  lite: ar ? "لايت" : "Lite",
                  pro: ar ? "برو" : "Pro",
                  infinity: ar ? "إنفينتي" : "Infinite",
                };
                const tierDescs = {
                  lite: ar ? "الباقة الأساسية مثالية للانطلاق" : "The essential plan for getting started",
                  pro: ar ? "الباقة الذكية الأكثر توازناً" : "The smartest balance of features",
                  infinity: ar ? "الباقة الشاملة بلا قيود" : "The complete plan without limits",
                };
                  ...
                  <motion.div key={tier} {...fade(i)} className={`flex flex-col relative rounded-2xl overflow-hidden border transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${
                    ...
                    <div className={`flex-1 px-6 py-4 ${isInf ? "bg-[#09090f]" : isPro ? "bg-[#1a3a6e]" : "bg-white dark:bg-[#0f172a]"}`}>
                      <ul className="space-y-2.5">
                        {(ar ? HOME_TIER_FEATURES[tier].ar : HOME_TIER_FEATURES[tier].en).map((f, fi) => (
                    ...
                    <div className={`px-6 pb-6 pt-4 ${isInf ? "bg-[#09090f]" : isPro ? "bg-[#1a3a6e]" : "bg-white dark:bg-[#0f172a]"}`}>
                      ...
                      >
                        ...
                        {ar ? "أكمل الطلب الآن" : "Complete your order"}
                      </button>
                      <p className={`text-[10px] text-center mt-2 ${isInf || isPro ? "text-white/30" : "text-gray-400/70"}`}>
                         {ar ? "تحويل بنكي · تواصل واتساب" : "Bank transfer · WhatsApp contact"}
                      </p>
                    </div>
                  </motion.div>
                );
            ...
            <motion.div {...fade(4)} className="mt-8 max-w-5xl mx-auto">
              <div className="rounded-2xl bg-black dark:bg-white p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative overflow-hidden">
            ...
            <motion.div {...fade(0)} className="flex items-start justify-between mb-10 md:mb-14" dir="ltr">
              ...
              <div className="text-left">
                <p className="text-[10px] font-black tracking-[0.2em] uppercase text-black/25 dark:text-white/25">
                  {customerCountData ? customerCountData.count : "—"} {ar ? "عميل يثق بنا" : "Trusted Clients"}
                </p>
              ...
              return (
                ...
                >
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {/* Transparent logo area: partner artwork stays clean without a tile background. */}
                      <div className="flex items-center justify-center bg-transparent p-10 md:p-14 min-h-[220px] relative overflow-hidden">
                        <img
                          src={activeP.logo}
                      ...
                      <button
                        ...
                        className={`h-11 px-3 rounded-xl border transition-all duration-300 flex items-center justify-center overflow-hidden ${
                          i === (activePartnerIdx % partners.length)
                            ? "border-black/15 dark:border-white/15 bg-transparent shadow-sm scale-105"
        ...
        <section className="relative overflow-hidden bg-white dark:bg-[#0f0f0f] border-t border-black/[0.06] dark:border-white/[0.06]">
          ...
          <div className="container mx-auto px-6 md:px-10 max-w-6xl relative">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-0 items-center min-h-[220px] py-12">

              {/* Text side */}
              <motion.div {...fade(0)} className={`flex flex-col justify-center gap-4 ${ar ? "text-right" : "text-left"}`} dir={ar ? "rtl" : "ltr"}>
                <h2 className="text-[2rem] md:text-[2.6rem] font-black leading-[1.1] tracking-tight text-black dark:text-white">
                  {ar
