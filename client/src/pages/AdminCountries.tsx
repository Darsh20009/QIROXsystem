    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">إدارة الدول</h1>
            <p className="text-sm text-muted-foreground">{countries.length} دولة مسجّلة</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="rounded-xl border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">إجمالي الدول</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <p className="text-2xl font-bold text-green-500">{activeCount}</p>
          <p className="text-xs text-muted-foreground mt-1">دول نشطة</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <p className="text-2xl font-bold text-red-400">{inactiveCount}</p>
          <p className="text-xs text-muted-foreground mt-1">دول موقوفة</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <p className="text-2xl font-bold text-blue-500">{continentGroups.length}</p>
          <p className="text-xs text-muted-foreground mt-1">قارات مغطّاة</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={filterContinent} onValueChange={setFilterContinent}>
          <SelectTrigger className="w-40" data-testid="select-filter-continent">
            <SelectValue placeholder="القارة" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">كل القارات</SelectItem>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
          ...
          <p>{countries.length === 0 ? "لا توجد دول، أضف الدول الافتراضية للبدء" : "لا توجد نتائج مطابقة"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(c => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-lg bg-muted/50 p-2">
                  <p className="text-muted-foreground">رمز الهاتف</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg" dir="rtl">
          <DialogHeader>
            <DialogTitle>{editingId ? "تعديل الدولة" : "إضافة دولة جديدة"}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>كود الدولة (ISO) <span className="text-red-500">*</span></Label>
          ...
          <div className="flex gap-2 pt-2">
            <Button className="flex-1" onClick={() => saveM.mutate(form)} disabled={saveM.isPending || !form.nameAr || !form.code} data-testid="btn-save-country">
              {saveM.isPending ? "جاري الحفظ..." : editingId ? "حفظ التعديلات" : "إضافة الدولة"}
            </Button>
            <Button variant="outline" onClick={() => setDialogOpen(false)} data-testid="btn-cancel-country">إلغاء</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
