...
import { Loader2, DollarSign, Download, RefreshCw, CheckCircle, Clock, Banknote, Plus, Briefcase, Trash2, Gift, TrendingUp, BadgeDollarSign, HelpCircle, Mail } from "lucide-react";
                ...
                <Card key={rec.id} className="border-black/[0.06] dark:border-white/[0.06] shadow-none rounded-2xl dark:bg-gray-900">
                  <CardContent className="pt-4 pb-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Button size="sm" variant="outline" className="h-8 text-xs gap-1 dark:text-white dark:border-white/10"
                        onClick={() => fetch(`/api/admin/payroll/${rec.id}/send-email`, { method: "POST", credentials: "include" }).then(r => r.json()).then(d => alert(d.ok ? (L ? "تم إرسال كشف الراتب بالبريد" : "Payslip sent!") : d.error)).catch(() => alert("Error"))}>
                        <Mail className="w-3 h-3" /> {L ? "إرسال كشف" : "Send Payslip"}
                      </Button>
