...
import { Loader2, LayoutGrid, RefreshCw, User, Calendar, Tag, ChevronRight, Plus, X, Database, Server, Globe, Code, GitBranch, Layers, ClipboardList, Trash2, AlertCircle, Pencil, Save, CheckCircle2, Search } from "lucide-react";
...
function TaskCard({ task, onMove, onDelete, onEdit, employees, readOnly = false }: {
  task: any;
  ...
  const [editPlan, setEditPlan] = useState({ ...WEBSITE_PLAN_DEFAULTS, ...(task.plan || {}) });
  ...
  useEffect(() => {
    if (showDetail) {
      ...
      setEditPlan({ ...WEBSITE_PLAN_DEFAULTS, ...(task.plan || {}) });
      setEditMode(false);
    }
  }, [showDetail, task]);
  ...
  const handleSave = () => {
    ...
    onEdit(task._id, {
      ...
      plan: isWebPlan ? editPlan : undefined,
    });
    setEditMode(false);
  };

  const FField = ({ label, children }: { label: string; children: JSX.Element | string | null }) => (
    <div>
      <p className="text-[10px] font-bold text-black/40 dark:text-white/40 uppercase tracking-widest mb-1">{label}</p>
      {children}
    </div>
  );
  ...
  return (
    <>
      <motion.div layout initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {!readOnly && <button onClick={e => { e.stopPropagation(); setShowDetail(true); setEditMode(true); }}
              className="p-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-950/30 text-blue-400 transition-colors"
              title={L ? "تعديل" : "Edit"} data-testid={`button-edit-task-${task._id}`}>
              <Pencil className="w-3 h-3" />
            </button>}
            <button onClick={e => { e.stopPropagation(); setShowMove(v => !v); }}
              className="p-1 rounded-lg hover:bg-black/[0.05] dark:hover:bg-white/[0.05] transition-colors"
        ...
        {task.assignedTo && (
          <div className="flex items-center gap-1.5 mb-2">
            <div className="w-4 h-4 rounded-full bg-black dark:bg-white flex items-center justify-center">
              <span className="text-white dark:text-black text-[8px] font-bold">{(task.assignedTo.fullName || task.assignedTo.username || "?")[0]}</span>
            </div>
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          {!readOnly && <button onClick={e => { e.stopPropagation(); if (confirm(L ? "حذف هذه المهمة؟" : "Delete this task?")) onDelete(task._id); }}
      ...
      <Dialog open={showDetail} onOpenChange={o => { setShowDetail(o); if (!o) setEditMode(false); }}>
          ...
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle className="flex items-center gap-2 text-base">
                ...
                {editMode ? (L ? "تعديل المهمة" : "Edit Task") : task.title}
              </DialogTitle>
              <div className="flex items-center gap-2">
                {!editMode ? (
                  !readOnly && <Button size="sm" variant="outline" className="h-7 text-xs gap-1" onClick={() => setEditMode(true)} data-testid="button-enter-edit-mode">
                    <Pencil className="w-3 h-3" /> {L ? "تعديل" : "Edit"}
                  </Button>
                ) : (
                  !readOnly && <div className="flex gap-2">
                    <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setEditMode(false)}>{L ? "إلغاء" : "Cancel"}</Button>
                    <Button size="sm" className="h-7 text-xs gap-1 bg-black dark:bg-white text-white dark:text-black" onClick={handleSave} data-testid="button-save-task">
                      <Save className="w-3 h-3" /> {L ? "حفظ" : "Save"}
                    </Button>
                  </div>
                )}
              </div>
              ...
              {editMode ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <FField label={L ? "عنوان المهمة *" : "Task Title *"}>
                      <Input value={editForm.title} onChange={e => setEditForm(f => ({ ...f, title: e.target.value }))}
                        className="h-9 text-sm" data-testid="input-edit-title" />
                    </FField>
                    <FField label={L ? "وصف" : "Description"}>
                      <Textarea value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))}
                        className="resize-none h-20 text-sm" data-testid="input-edit-description" />
                    </FField>
                    <div className="grid grid-cols-2 gap-3">
                      <FField label={L ? "الأولوية" : "Priority"}>
                        <Select value={editForm.priority} onValueChange={v => setEditForm(f => ({ ...f, priority: v }))}>
                          <SelectTrigger className="h-9" data-testid="select-edit-priority"><SelectValue /></SelectTrigger>
                          <SelectContent>
                      ...
                      <FField label={L ? "تاريخ الانتهاء" : "Deadline"}>
                        <Input type="date" value={editForm.deadline} onChange={e => setEditForm(f => ({ ...f, deadline: e.target.value }))}
                          className="h-9 text-sm" data-testid="input-edit-deadline" />
                      </FField>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <FField label={L ? "المُكلَّف" : "Assigned To"}>
                        <Select value={editForm.assignedTo || "none"} onValueChange={v => setEditForm(f => ({ ...f, assignedTo: v === "none" ? "" : v }))}>
                          <SelectTrigger className="h-9" data-testid="select-edit-assigned"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="none">{L ? "— بدون تكليف —" : "— Unassigned —"}</SelectItem>
                            {employees.map((e: any) => (
                              <SelectItem key={e._id || e.id} value={e._id || e.id}>{e.fullName || e.username}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </FField>
                      <FField label={L ? "اسم المشروع" : "Project Name"}>
                        <Input value={editForm.projectName} onChange={e => setEditForm(f => ({ ...f, projectName: e.target.value }))}
                          className="h-9 text-sm" placeholder={L ? "اختياري" : "Optional"} />
                      </FField>
                    </div>
                  </div>
                  ...
                  {/* Website plan fields */}
                  {isWebPlan && (
                    <div className="border-t border-black/[0.06] dark:border-white/[0.06] pt-4 space-y-3">
                      <p className="text-[10px] font-black text-black/40 dark:text-white/40 uppercase tracking-widest flex items-center gap-1.5">
                        <Globe className="w-3 h-3" /> {L ? "بيانات خطة الموقع" : "Website Plan Details"}
                      </p>
                      <FField label={L ? "فكرة المشروع" : "Project Concept"}>
                        <Textarea value={editPlan.projectConcept} onChange={e => setEditPlan(p => ({ ...p, projectConcept: e.target.value }))}
                          className="resize-none h-20 text-sm" data-testid="input-edit-concept" />
                      </FField>
                      <div className="grid grid-cols-2 gap-3">
                        <FField label={L ? "التقنيات" : "Tech Stack"}>
                          <Input value={editPlan.techStack} onChange={e => setEditPlan(p => ({ ...p, techStack: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label={L ? "الإطار" : "Framework"}>
                          <Input value={editPlan.framework} onChange={e => setEditPlan(p => ({ ...p, framework: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label={L ? "قاعدة البيانات" : "Database"}>
                          <Input value={editPlan.database} onChange={e => setEditPlan(p => ({ ...p, database: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label={L ? "الاستضافة" : "Hosting"}>
                          <Input value={editPlan.hosting} onChange={e => setEditPlan(p => ({ ...p, hosting: e.target.value }))} className="h-9 text-sm" />
                        </FField>
                        <FField label={L ? "النطاق" : "Domain"}>
                          <Input value={editPlan.domain} onChange={e => setEditPlan(p => ({ ...p, domain: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label="GitHub Repo">
                          <Input value={editPlan.githubRepo} onChange={e => setEditPlan(p => ({ ...p, githubRepo: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label="Staging URL">
                          <Input value={editPlan.stagingUrl} onChange={e => setEditPlan(p => ({ ...p, stagingUrl: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                        <FField label="Production URL">
                          <Input value={editPlan.productionUrl} onChange={e => setEditPlan(p => ({ ...p, productionUrl: e.target.value }))} className="h-9 text-sm font-mono" />
                        </FField>
                      </div>
                      <FField label={L ? "تصميم قاعدة البيانات" : "Database Design"}>
                        <Textarea value={editPlan.databaseDesign} onChange={e => setEditPlan(p => ({ ...p, databaseDesign: e.target.value }))}
                          className="resize-none h-24 text-xs font-mono" />
                      </FField>
                      <FField label={L ? "استراتيجية النشر" : "Deployment Strategy"}>
                        <Textarea value={editPlan.deploymentStrategy} onChange={e => setEditPlan(p => ({ ...p, deploymentStrategy: e.target.value }))}
                          className="resize-none h-20 text-sm" />
                      </FField>
                      <FField label={L ? "المميزات الأساسية" : "Main Features"}>
                        <Textarea value={editPlan.mainFeatures} onChange={e => setEditPlan(p => ({ ...p, mainFeatures: e.target.value }))}
                          className="resize-none h-20 text-sm" />
                      </FField>
                      <FField label={L ? "ملاحظات" : "Notes"}>
                        <Textarea value={editPlan.notes} onChange={e => setEditPlan(p => ({ ...p, notes: e.target.value }))}
                          className="resize-none h-20 text-sm" />
                      </FField>
                    </div>
                  )}
                ...
                <div className="space-y-3">
                  {/* Meta info bar */}
                  <div className="flex flex-wrap items-center gap-2 pb-3 border-b border-black/[0.05] dark:border-white/[0.05]">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${priorityColors[task.priority] || priorityColors.medium}`}>
                      {L ? priorityLabels[task.priority] : task.priority}
                    </span>
                    {task.assignedTo && (
                      <span className="flex items-center gap-1 text-[10px] text-black/60 dark:text-white/60 bg-black/[0.04] dark:bg-white/[0.04] px-2 py-0.5 rounded-full">
                        <User className="w-3 h-3" />{task.assignedTo.fullName || task.assignedTo.username}
                      </span>
                    )}
                    {task.deadline && (
                      <span className="flex items-center gap-1 text-[10px] text-black/60 dark:text-white/60 bg-black/[0.04] dark:bg-white/[0.04] px-2 py-0.5 rounded-full">
                        <Calendar className="w-3 h-3" />{new Date(task.deadline).toLocaleDateString(L ? "ar-SA" : "en-US")}
                      </span>
                    )}
                    {task.projectName && (
                      <span className="flex items-center gap-1 text-[10px] text-black/60 dark:text-white/60 bg-black/[0.04] dark:bg-white/[0.04] px-2 py-0.5 rounded-full">
                        <Tag className="w-3 h-3" />{task.projectName}
                      </span>
                    )}
                    <span className="flex items-center gap-1 text-[10px] text-black/40 dark:text-white/40 bg-black/[0.04] dark:bg-white/[0.04] px-2 py-0.5 rounded-full">
                      <CheckCircle2 className="w-3 h-3" />{stages.find(s => s.key === task.status)?.label || task.status}
                    </span>
                  </div>
                  {task.description && (
                    <p className="text-sm text-black/60 dark:text-white/60 leading-relaxed">{task.description}</p>
                  )}
                  {isWebPlan && task.plan && (
                    <div className="grid grid-cols-2 gap-3">
                      {task.plan.projectConcept && (
                        <div className="col-span-2 bg-black/[0.03] dark:bg-white/[0.03] rounded-xl p-3 border border-black/[0.06]">
                          ...
                          <p className="text-sm text-black dark:text-white whitespace-pre-wrap">{task.plan.projectConcept}</p>
                        </div>
                      )}
                      {task.plan.techStack && (
                      ...
                      {task.plan.databaseDesign && (
                        <div className="col-span-2 bg-black/[0.03] dark:bg-white/[0.03] rounded-xl p-3 border border-black/[0.06]">
                          <p className="text-[10px] font-bold text-black/40 uppercase mb-1 flex items-center gap-1"><Database className="w-3 h-3" /> {L ? "تصميم قاعدة البيانات" : "DB Design"}</p>
                      ...
                      {task.plan.deploymentStrategy && (
                        <div className="col-span-2 bg-black/[0.03] dark:bg-white/[0.03] rounded-xl p-3 border border-black/[0.06]">
                          <p className="text-[10px] font-bold text-black/40 uppercase mb-1 flex items-center gap-1"><GitBranch className="w-3 h-3" /> {L ? "استراتيجية النشر" : "Deployment"}</p>
                      ...
                      {(task.plan.domain || task.plan.githubRepo || task.plan.stagingUrl || task.plan.productionUrl) && (
                        <div className="col-span-2 bg-black/[0.03] dark:bg-white/[0.03] rounded-xl p-3 border border-black/[0.06]">
                          ...
                          <div className="space-y-1">
                            {task.plan.domain && <p className="text-xs text-black/70 dark:text-white/70 font-mono">🌐 {task.plan.domain}</p>}
                            {task.plan.githubRepo && <p className="text-xs text-black/70 dark:text-white/70 font-mono">⌨️ {task.plan.githubRepo}</p>}
                            {task.plan.stagingUrl && <p className="text-xs text-black/70 dark:text-white/70 font-mono">🔧 {task.plan.stagingUrl}</p>}
                            {task.plan.productionUrl && <p className="text-xs text-black/70 dark:text-white/70 font-mono">🚀 {task.plan.productionUrl}</p>}
                          </div>
                        </div>
                      )}
                      {task.plan.notes && (
...
export default function AdminKanban() {
  ...
  const isDataEntry = (currentUser as any)?.role === "data_entry";
  const [createOpen, setCreateOpen] = useState(false);
  const [templateType, setTemplateType] = useState<"custom" | "website_plan" | "development">("custom");
  ...
  const [devForm, setDevForm] = useState({ techDescription: "", language: "", githubRepo: "", checklist: "", notes: "" });
  ...
  const deleteTaskMutation = useMutation({
    ...
    onError: () => toast({ title: L ? "فشل حذف المهمة" : "Delete failed", variant: "destructive" }),
  });

  const editTaskMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      apiRequest("PATCH", `/api/admin/kanban/tasks/${id}`, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/admin/kanban/tasks"] });
      toast({ title: L ? "✅ تم تحديث المهمة" : "✅ Task updated" });
    },
  ...
  const createTaskMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDevForm({ techDescription: "", language: "", githubRepo: "", checklist: "", notes: "" });
  ...
  const handleCreate = () => {
    ...
    createTaskMutation.mutate({
      ...
      devPlan: templateType === "development" ? devForm : undefined,
    });
  };
  ...
  let filteredProjects = searchLower ? projects.filter((p: any) => (p.title || p.service || "").toLowerCase().includes(searchLower)) : projects;
  let filteredTasks = searchLower ? tasks.filter((t: any) => (t.title || "").toLowerCase().includes(searchLower) || (t.description || "").toLowerCase().includes(searchLower)) : tasks;
  if (myTasksOnly && myId) {
  ...
  const groupedProjects = STAGES.reduce((acc, s) => {
    acc[s.key] = filteredProjects.filter((p: any) => p.status === s.key);
    return acc;
  }, {} as Record<string, any[]>);

  const groupedTasks = STAGES.reduce((acc, s) => {
    acc[s.key] = filteredTasks.filter((t: any) => t.status === s.key);
    return acc;
  }, {} as Record<string, any[]>);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <Search className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-black/30 dark:text-white/30 pointer-events-none" />
            <Input
              value={taskSearch}
              onChange={e => setTaskSearch(e.target.value)}
              placeholder={L ? "بحث..." : "Search..."}
              className="h-8 text-xs pr-8 w-40 bg-black/[0.03] dark:bg-white/[0.04] border-black/[0.07] dark:border-white/[0.07]"
              data-testid="input-kanban-search"
            />
          </div>
          <button
            onClick={() => setMyTasksOnly(v => !v)}
            className={`h-8 px-3 text-xs rounded-lg border transition-colors font-bold ${myTasksOnly ? "bg-black dark:bg-white text-white dark:text-black border-black dark:border-white" : "border-black/[0.10] dark:border-white/[0.10] text-black/50 dark:text-white/50 hover:border-black/20 dark:hover:border-white/20"}`}
            data-testid="button-my-tasks-only"
          >
            {L ? "مهامي فقط" : "My Tasks"}
          </button>
          <Button variant="ghost" size="sm" onClick={() => { refetch(); refetchTasks(); }} data-testid="button-refresh-kanban">
            <RefreshCw className="w-3.5 h-3.5" />
          </Button>
          {!isDataEntry && <Button size="sm" className="bg-black text-white dark:bg-white dark:text-black h-8 text-xs gap-1.5"
                  ...
                  <AnimatePresence>
                    ...
                    {taskCols.map((task: any) => (
                      <TaskCard key={task._id} task={task}
                        ...
                        onEdit={(id, data) => editTaskMutation.mutate({ id, data })}
                        readOnly={isDataEntry}
                        employees={employees} />
                    ))}
                  </AnimatePresence>
                  {total === 0 && (
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button onClick={() => setTemplateType("website_plan")}
                    ...
                    <p className="text-xs font-bold">{L ? "خطة موقع" : "Website Plan"}</p>
                    <p className="text-[10px] opacity-60">{L ? "قاعدة بيانات · نشر" : "DB · Deployment"}</p>
                  </button>
                  <button onClick={() => setTemplateType("development")}
                    className={`p-3 rounded-xl border text-right transition-colors ${templateType === "development" ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-black/10 bg-black/[0.02] dark:border-white/10 dark:bg-white/[0.02] text-black dark:text-white hover:bg-black/[0.05]"}`}
                    data-testid="button-template-development">
                    <Code className="w-4 h-4 mb-1" />
                    <p className="text-xs font-bold">{L ? "مهمة تطوير" : "Dev Task"}</p>
                    <p className="text-[10px] opacity-60">{L ? "برمجة · تقنية" : "Code · Technical"}</p>
                  </button>
                </div>
              </div>
              ...
              {templateType === "development" && (
                <div className="space-y-4 pt-2 border-t border-black/[0.06] dark:border-white/[0.06]">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-bold text-black/60 dark:text-white/60 uppercase tracking-widest">{L ? "تفاصيل مهمة التطوير" : "Development Task Details"}</p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-black/40 uppercase mb-1.5 block">{L ? "وصف تقني مفصّل" : "Technical Description"}</label>
                    <Textarea value={devForm.techDescription} onChange={e => setDevForm(f => ({ ...f, techDescription: e.target.value }))}
                      placeholder={L ? "اشرح المهمة التقنية بشكل تفصيلي..." : "Describe the technical task in detail..."}
                      className="resize-none h-20" data-testid="input-dev-description" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-black/40 uppercase mb-1.5 flex items-center gap-1"><Code className="w-3 h-3" /> {L ? "اللغة / التقنية" : "Language / Stack"}</label>
                      <Input value={devForm.language} onChange={e => setDevForm(f => ({ ...f, language: e.target.value }))}
                        placeholder="TypeScript / React / Python" className="h-9 font-mono text-sm" data-testid="input-dev-language" />
                    </div>
                    <div>
                      ...
                      <Input value={devForm.githubRepo} onChange={e => setDevForm(f => ({ ...f, githubRepo: e.target.value }))}
                        placeholder="github.com/user/repo" className="h-9 font-mono text-sm" data-testid="input-dev-github" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-black/40 uppercase mb-1.5">{L ? "قائمة مهام تقنية (Checklist)" : "Technical Checklist"}</label>
                    <Textarea value={devForm.checklist} onChange={e => setDevForm(f => ({ ...f, checklist: e.target.value }))}
                      placeholder={L ? "- إنشاء API endpoint\n- ربط قاعدة البيانات\n- كتابة اختبارات الوحدة" : "- Create API endpoint\n- Database integration\n- Write unit tests"}
                      className="resize-none h-24 font-mono text-sm" data-testid="input-dev-checklist" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-black/40 uppercase mb-1.5">{L ? "ملاحظات تقنية" : "Technical Notes"}</label>
                    <Textarea value={devForm.notes} onChange={e => setDevForm(f => ({ ...f, notes: e.target.value }))}
                      placeholder={L ? "أي ملاحظات أو متطلبات تقنية إضافية..." : "Any additional technical notes or requirements..."}
                      className="resize-none h-16" data-testid="input-dev-notes" />
                  </div>
                </div>
              )}
