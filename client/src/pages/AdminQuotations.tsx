...
function QuotationForm({ onClose }: { onClose: () => void }) {
  ...
  const vatAmt = Math.round(afterDiscount * (Number(form.vatRate) / 100) * 100) / 100;
  ...
  return (
    <div className="space-y-4 max-h-[75vh] overflow-y-auto pr-1" dir={dir}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <Label className="text-xs text-black/50 mb-1 block">{L ? "ضريبة %" : "VAT %"}</Label>
          <Input type="number" min="0" max="100" value={form.vatRate} onChange={e => setForm(p => ({ ...p, vatRate: e.target.value }))}
            className="h-9 text-sm border-black/[0.10]" dir="ltr" data-testid="input-quotation-vat" />
        </div>
        <div>
          <Label className="text-xs text-black/50 mb-1 block">{L ? "خصم %" : "Discount %"}</Label>
          <Input type="number" min="0" max="100" value={form.discountPercent} onChange={e => setForm(p => ({ ...p, discountPercent: e.target.value }))}
            className="h-9 text-sm border-black/[0.10]" dir="ltr" data-testid="input-quotation-discount" />
        </div>
        <div>
...
export default function AdminQuotations() {
  ...
  const canSeeFinance = ["admin", "manager", "owner", "accountant"].includes((currentUser as any)?.role || "");
                    ...
                    <div className="text-left">
                      <div className="text-sm font-black text-black flex items-center gap-1">
                        {canSeeFinance
                          ...
                          : <span className="tracking-widest text-black/40">****</span>}
                      </div>
                      {q.vatRate > 0 && <div className="text-xs text-black/30">{canSeeFinance ? `شامل ضريبة ${q.vatRate}%` : ""}</div>}
                    </div>
