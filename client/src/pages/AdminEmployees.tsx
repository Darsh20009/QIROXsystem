...
function getRoleLabels(L: boolean): Record<string, string> {
  return L ? {
    ...
    sales: "مبيعات", developer: "مطور", designer: "مصمم", support: "دعم فني", data_entry: "إدخال البيانات", merchant: "توصيل", client: "عميل",
  } : {
    admin: "System Admin", manager: "Manager", accountant: "Accountant", sales_manager: "Sales Manager",
    sales: "Sales", developer: "Developer", designer: "Designer", support: "Support", data_entry: "Data Entry", merchant: "Delivery", client: "Client",
  };
}
...
const ROLE_HIERARCHY: { roles: string[]; level: number; icon: any; color: string; bg: string; label: string; labelEn: string }[] = [
  ...
  { roles: ["sales", "support", "data_entry", "merchant"],level: 4, icon: Headphones, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50/50 dark:bg-emerald-900/15 border-emerald-200/50 dark:border-emerald-700/30", label: "الفريق التشغيلي", labelEn: "Operations" },
];
...
const roleColors: Record<string, string> = {
  ...
  data_entry: "bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white border-black/10 dark:border-white/10",
...
const employeeRoles = ["manager", "accountant", "sales_manager", "sales", "developer", "designer", "support", "data_entry", "merchant", "hr", "marketing", "content"];
...
export default function AdminEmployees() {
  ...
  const canManageStaff = ["admin", "manager"].includes((currentUser as any)?.role);
  ...
  const canDeleteUser = (user: User) =>
    ...
    ((currentUser as any)?.role === "admin" ||
      ((currentUser as any)?.role === "manager" && user.role !== "manager"));
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<EmployeeForm> }) => {
      const { salaryType, fixedSalary, hourlyRate, commissionRate, workEmailAddress, createWorkEmail, ...userFields } = data as any;
      if (workEmailAddress !== undefined) {
        userFields.workEmail = String(workEmailAddress || "").trim().toLowerCase();
      }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_response, deletedId) => {
      queryClient.setQueryData<User[]>(["/api/admin/users"], (currentUsers) =>
        currentUsers?.filter((user) => getUserId(user) !== deletedId),
      );
      queryClient.invalidateQueries({ queryKey: ["/api/admin/users"] });
  ...
  const startEdit = (emp: User) => {
    ...
    setForm({
      ...
      createWorkEmail: false,
      workEmailAddress: (emp as any).workEmail || "",
  ...
  const handleDelete = (emp: User) => {
    if (!canDeleteUser(emp)) return;
    const id = getUserId(emp);
    if (!id) {
      toast({ title: L ? "تعذر تحديد الموظف" : "Employee ID is missing", variant: "destructive" });
      return;
    }
    setDeleteTarget(emp);
  };
    ...
    <div className="relative overflow-hidden p-4 md:p-8 max-w-[1200px] mx-auto space-y-6">
            ...
            <Card className="border border-black/[0.06] shadow-none">
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {!isDataEntry && form.salaryType === "hourly" && (
                    <div>
                  ...
                  {!isDataEntry && form.salaryType === "fixed" && (
                    <div>
                  ...
                  {!isDataEntry && form.salaryType === "commission" && (
                    <div>
                ...
                {/* Work email: create it for a new employee, or link/update it while editing */}
                {canManageStaff && (
                  <div className="mt-4 pt-4 border-t border-black/[0.06]">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-bold text-black/50">
                          {editingId
                            ? (L ? "البريد المؤسسي للموظف" : "Employee work email")
                            : (L ? "إنشاء بريد عمل للموظف" : "Create work email for employee")}
                        </span>
                      </div>
                      {!editingId && (
                        <button
                    ...
                    <div className="mt-3">
                      <Label className="text-[11px] text-black/40 mb-1.5 block">
                        {editingId
                          ? (L ? "عنوان البريد المؤسسي (يمكن تركه فارغاً لإزالته)" : "Work email address (leave blank to remove)")
                          : (L ? "عنوان البريد المؤسسي" : "Work Email Address")}
                      </Label>
                      <div className="flex items-center gap-2">
                        <Input
                          dir="ltr"
                          ...
                          onChange={e => setForm(f => ({
                            ...
                            createWorkEmail: editingId ? f.createWorkEmail : Boolean(e.target.value.trim()),
                          }))}
                        ...
                        <button
                          ...
                          onClick={() => setForm(f => ({
                            ...f,
                            createWorkEmail: true,
                            workEmailAddress: `${f.username}@qirox.online`,
                          }))}
                      ...
                      <p className="text-[10px] text-black/30 mt-1.5 flex items-center gap-1">
                        ...
                        {editingId
                          ? (L ? "سيتم حفظ البريد ضمن ملف الموظف ويمكن إدارته من حسابات البريد" : "Saved on the employee profile and can be managed from Email Accounts")
                          : (L ? "عند تعبئة الخانة سيتم إنشاء الحساب تلقائياً في نظام البريد باعتماد بيانات cPanel" : "Filling this field creates the mailbox automatically using cPanel credentials")}
                      </p>
                    </div>
                  </div>
                )}
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent dir={dir}>
          <AlertDialogHeader>
            <AlertDialogTitle>{L ? "حذف الموظف" : "Delete Employee"}</AlertDialogTitle>
            <AlertDialogDescription>
              {L ? "هل أنت متأكد من حذف حساب" : "Are you sure you want to delete"}
              {" "}
              ...
              <span className="text-red-600 text-xs mt-1 inline-block">
                {L ? "لا يمكن التراجع عن هذا الإجراء." : "This action cannot be undone."}
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-row-reverse gap-2">
            <AlertDialogCancel data-testid="button-cancel-delete-employee">
              {L ? "إلغاء" : "Cancel"}
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                const id = deleteTarget ? getUserId(deleteTarget) : "";
            ...
            >
              ...
              {L ? "حذف الموظف نهائياً" : "Delete Permanently"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
