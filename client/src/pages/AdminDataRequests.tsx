      ...
      <DialogContent className="max-w-xl max-h-[92vh]" dir={dir}>
        ...
        <ScrollArea className="max-h-[60vh]">
          <div className="space-y-5 px-1 pb-2">
            ...
            {/* Action buttons — shown for submitted or pending */}
            {(req.status === "submitted" || req.status === "pending") && (
              <div className="grid grid-cols-2 gap-3">
                <Button className="bg-black dark:bg-white hover:bg-black/80 dark:hover:bg-white/80 text-white dark:text-black font-bold h-11 rounded-xl gap-2"
                  onClick={() => updateMutation.mutate("approved")} disabled={updateMutation.isPending}
                  data-testid="button-dr-approve">
                  <CheckCircle2 className="w-4 h-4" /> {L ? "إغلاق وموافقة" : "Close & Approve"}
                </Button>
                ...
                {req.status === "pending" && (
                  <Button variant="outline" className="h-11 rounded-xl gap-2 border-black/20 dark:border-white/20"
                    onClick={() => updateMutation.mutate("revision_needed")} disabled={updateMutation.isPending}
                    data-testid="button-dr-pending-revision">
                    <RotateCcw className="w-4 h-4" /> {L ? "إرجاع للعميل" : "Return to Client"}
                  </Button>
                )}
              </div>
            )}
