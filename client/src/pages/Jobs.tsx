...
export default function Jobs() {
  ...
  const allOpenJobs = jobs.filter(j => j.status === "open");
  const openJobs = jobTypeFilter === "all" ? allOpenJobs : allOpenJobs.filter(j => j.type === jobTypeFilter);
  const jobTypes = Array.from(new Set(allOpenJobs.map(j => j.type).filter(Boolean)));
  ...
  const GENERAL_JOB: Job = {
    ...
    requirements: "",
    location: lang === "ar" ? "الرياض، المملكة العربية السعودية" : "Riyadh, Saudi Arabia",
    type: "full-time",
    salaryRange: "",
    status: "open",
  ...
  useSEO({
    ...
    keywords: "وظائف كيروكس استوديو, وظائف برمجة الرياض, Qirox jobs, وظائف تقنية السعودية, مطلوب مطور مواقع, وظائف تصميم الرياض, Qirox Studio careers, وظائف شركة برمجة, وظائف ريادة أعمال السعودية",
    canonical: "/jobs",
    jsonLd: openJobs.length > 0 ? openJobs.map(job => ({
      "@context": "https://schema.org",
      "@type": "JobPosting",
      "title": job.title,
      "description": job.description,
      "datePosted": job.createdAt?.split("T")[0],
      "employmentType": job.type === "full-time" ? "FULL_TIME" : job.type === "part-time" ? "PART_TIME" : job.type === "remote" ? "TELECOMMUTE" : "CONTRACTOR",
      "hiringOrganization": {
        "@type": "Organization",
        "name": "Qirox Studio",
      ...
      "url": "https://qiroxstudio.online/jobs",
    })) : [{
      "@context": "https://schema.org",
      "@type": "JobPosting",
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            {allOpenJobs.length > 0 && (
              <div className="inline-flex items-center gap-2 bg-black/[0.04] dark:bg-white/[0.06] px-4 py-1.5 rounded-full text-sm font-bold text-black dark:text-white">
                <span className="w-2 h-2 bg-black dark:bg-white rounded-full animate-pulse" />
                {allOpenJobs.length} {lang === "ar" ? "وظيفة مفتوحة" : "open positions"}
              </div>
            )}
            {jobTypes.length > 1 && (
              <div className="flex gap-2 flex-wrap justify-center mt-4">
                {["all", ...jobTypes].map(t => {
                  const typeLabels: Record<string, string> = { all: lang === "ar" ? "الكل" : "All", "full-time": lang === "ar" ? "دوام كامل" : "Full-time", "part-time": lang === "ar" ? "دوام جزئي" : "Part-time", remote: lang === "ar" ? "عن بُعد" : "Remote", freelance: lang === "ar" ? "فريلانس" : "Freelance", internship: lang === "ar" ? "تدريب" : "Internship" };
                  return (
                    <button key={t} onClick={() => setJobTypeFilter(t)}
                      className={`px-4 py-1.5 rounded-full text-xs font-bold border transition-all ${jobTypeFilter === t ? "bg-black dark:bg-white text-white dark:text-black border-transparent" : "border-black/10 dark:border-white/10 text-black/50 dark:text-white/40 hover:border-black/25 dark:hover:border-white/25"}`}>
                      {typeLabels[t] || t}
                    </button>
                  );
                })}
