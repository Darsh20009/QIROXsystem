...
const STATUS_MAP = {
  pending:        { label: "جديد", color: "bg-amber-100 text-amber-700 border-amber-200", dot: "bg-amber-500" },
  contacted:      { label: "تم التواصل", color: "bg-blue-100 text-blue-700 border-blue-200", dot: "bg-blue-500" },
  converted:      { label: "تحوّل لعميل", color: "bg-green-100 text-green-700 border-green-200", dot: "bg-green-500" },
  not_interested: { label: "غير مهتم", color: "bg-gray-100 text-gray-600 border-gray-200", dot: "bg-gray-400" },
};
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-black rounded-3xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-white">تذكيرات التحويل</h1>
              <p className="text-white/40 text-sm">عملاء سيتجدد اشتراكهم — فرصتك للتواصل قبل المنافسة</p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { val: stats.total, label: "إجمالي", color: "text-white" },
              { val: stats.pending, label: "جديد", color: "text-amber-400" },
              { val: stats.urgent, label: "عاجل (30 يوم)", color: "text-red-400" },
              { val: stats.converted, label: "تحوّل", color: "text-green-400" },
            ].map(({ val, label, color }) => (
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم أو الجوال أو الشركة..." className="pr-10 rounded-xl" data-testid="input-search" />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 border border-black/[0.06] dark:border-white/[0.06] rounded-3xl">
          ...
          <p className="text-black/30 dark:text-white/30 font-medium">لا توجد تذكيرات بعد</p>
          <p className="text-black/20 dark:text-white/20 text-sm mt-1">ستظهر هنا عندما يُسجّل العملاء طلبات التذكير</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-left flex-shrink-0">
                    ...
                    <p className="text-[10px] text-black/30 dark:text-white/30 text-center mt-1">
                      {new Date(r.subscriptionEndDate).toLocaleDateString("ar-SA", { month: "short", day: "numeric", year: "2-digit" })}
                    </p>
                  </div>
                </div>
                {r.notes && (
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-lg rounded-3xl" dir="rtl">
          {selected && (
            <>
              <DialogHeader>
                <DialogTitle className="text-right font-black text-lg">تفاصيل التذكير</DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-black/[0.02] dark:bg-white/[0.02] rounded-2xl p-4 space-y-2.5">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-black/60 dark:text-white/60">ينتهي في:</span>
                    <span className={`font-bold px-2 py-0.5 rounded-lg text-xs border ${urgencyColor(daysUntil(selected.subscriptionEndDate))}`}>
                      {new Date(selected.subscriptionEndDate).toLocaleDateString("ar-SA", { weekday: "short", year: "numeric", month: "long", day: "numeric" })}
                      {" "}({daysUntil(selected.subscriptionEndDate)} يوم)
                    </span>
                  </div>
                  {selected.notes && (
                    <div className="pt-2 border-t border-black/[0.06] dark:border-white/[0.06]">
                      <p className="text-xs text-black/40 dark:text-white/40 mb-1">ملاحظة العميل:</p>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-bold text-black/40 dark:text-white/40 mb-1.5 block">تحديث الحالة</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-black/40 dark:text-white/40 mb-1.5 block">ملاحظات الفريق</label>
                    <Textarea
                      value={editNotes}
                      onChange={e => setEditNotes(e.target.value)}
                      placeholder="سجّل ملاحظاتك بعد التواصل مع العميل..."
                      rows={3}
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    onClick={() => { if (confirm("حذف هذا التذكير؟")) deleteMutation.mutate(selected.id); }}
