...
const EMPTY_FORM = { companyName: "", contactName: "", phone: "", email: "", sector: "", source: "manual", notes: "", reminderAt: "" };
...
export default function EmployeeLeadsData() {
  ...
  function openLead(lead: Lead) {
    ...
    setEditForm({
      ...
      source: lead.source,
      notes: lead.notes,
      reminderAt: lead.reminderAt ? lead.reminderAt.slice(0, 10) : "",
    });
  }
            ...
            <>
              ...
              {isEditing ? (
                <div className="space-y-3 mb-5 p-4 bg-muted/50 rounded-xl border">
                  ...
                  <div>
                    <Label className="text-xs flex items-center gap-1"><Bell className="w-3 h-3" /> تاريخ التذكير</Label>
                    <input
                      type="date"
                      value={editForm.reminderAt || ""}
                      onChange={e => setEditForm(f => ({ ...f, reminderAt: e.target.value }))}
    ...
    >
      ...
      <div className="flex flex-wrap gap-1 mb-2">
        ...
        {lead.reminderAt && new Date(lead.reminderAt) < new Date() && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 font-bold">
            <Bell className="w-3 h-3" /> {lang === "ar" ? "تذكير متأخر" : "Overdue"}
          </span>
        )}
        {lead.sector && (
