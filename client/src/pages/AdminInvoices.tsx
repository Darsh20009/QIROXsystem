    ...
    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1" dir={dir}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input type="number" min="0" max="100" value={form.discountPercent} onChange={e => setForm(p => ({ ...p, discountPercent: e.target.value }))} className="h-9 text-sm border-black/[0.10]" placeholder="0" />
        </div>
      </div>
                    ...
                    <tr key={inv.id} className="border-b border-black/[0.05] hover:bg-black/[0.01] transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-bold text-black text-xs flex items-center gap-1">
                          {canSeeFinance
                            ? <>{inv.totalAmount.toLocaleString()} <SARIcon size={9} className="opacity-70" /></>
