...
export default function Cart() {
  ...
  /* payment states */
  const [paymentOption, setPaymentOption] = useState<"bank" | "paypal">("bank");
  ...
  const updateQtyMutation = useMutation<unknown, Error, { itemId: string; qty: number }>({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/cart"] }),
    onError: () => toast({ title: L ? "فشل تحديث الكمية" : "Qty update failed", variant: "destructive" }),
  });
  ...
  const addItemMutation = useMutation({
    ...
    onError: (e: any) => toast({ title: e?.message || (L ? "فشل إضافة العنصر" : "Add item failed"), variant: "destructive" }),
  });

  const clearMutation = useMutation({
    mutationFn: async () => { await apiRequest("DELETE", "/api/cart"); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/cart"] }); },
    onError: () => toast({ title: L ? "فشل مسح السلة" : "Clear cart failed", variant: "destructive" }),
  });
  ...
  const checkoutMutation = useMutation({
    mutationFn: async (walletPin?: string) => {
      ...
      const paymentMethod = fullyPaidByWallet ? "wallet" : walletUsed > 0 ? "mixed" : paymentOption === "paypal" ? "paypal" : "bank_transfer";
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <button
                          ...
                          <p className={`font-bold text-[10px] text-center ${paymentOption === "paypal" ? "text-[#003087]" : "text-black/60"}`}>PayPal / بطاقة</p>
                        </button>
                      </div>
