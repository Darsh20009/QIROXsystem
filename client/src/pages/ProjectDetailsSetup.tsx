    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg" dir="rtl">
        ...
        <div className="space-y-4 mt-2">
          ...
          <Button onClick={generate} disabled={loading} className="w-full gap-2 rounded-xl bg-black dark:bg-white hover:bg-black dark:bg-white font-bold" data-testid="ai-generate-btn">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
              <div className="bg-black/[0.04] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 rounded-xl p-4">
                ...
                <Textarea
                  ...
                  rows={5}
                  className="rounded-xl resize-none text-sm border-black/10 dark:border-white/10 bg-white"
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => { onApply(generated); onClose(); }} className="flex-1 gap-1.5 rounded-xl bg-black dark:bg-white hover:bg-black dark:bg-white font-bold" data-testid="ai-apply-btn">
...
export default function ProjectDetailsSetup() {
  ...
  if (saved) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black/[0.04] dark:from-white/[0.06] to-white flex items-center justify-center p-4" dir={dir}>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          <div className="w-24 h-24 bg-black/[0.04] dark:bg-white/[0.06] rounded-full flex items-center justify-center mx-auto mb-5">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black/[0.04] dark:from-white/[0.06] via-white to-black/[0.04] dark:to-white/[0.06]" dir={dir}>
      ...
      {/* Hero header */}
      <div className="bg-gradient-to-r from-black dark:from-white to-black dark:to-white px-5 pt-safe-top pb-8">
              ...
              <div key={i} className={`flex items-center gap-1.5 ${i < SECTIONS.length - 1 ? "flex-1" : ""}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  ...
                  {i < section
                    ...
                    : <s.icon className={`w-3.5 h-3.5 ${i === section ? "text-black dark:text-white" : "text-white/50"}`} />
                  }
                </div>
                {i < SECTIONS.length - 1 && (
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="s0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
              <div className="bg-white rounded-3xl border border-black/[0.06] p-5 shadow-sm">
                <h2 className="font-black text-black mb-4 flex items-center gap-2">
                  <Building2 className="w-5 h-5 text-black dark:text-white" /> بيانات النشاط
                </h2>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                      {sectors.length > 0 ? sectors.map((s: any) => (
                        <button key={s.value} onClick={() => set("sector", s.value)}
                          ...
                          className={`border-2 rounded-2xl p-3 text-center transition-all ${form.sector === s.value ? "border-black dark:border-white bg-black/[0.04] dark:bg-white/[0.06] shadow-sm" : "border-black/[0.07] hover:border-black/20"}`}>
              ...
              <Button onClick={() => setSection(1)} disabled={!canNext0}
                className="w-full h-12 rounded-2xl bg-black dark:bg-white hover:bg-black dark:bg-white text-white font-bold gap-2" data-testid="setup-btn-next-0">
          ...
          {section === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
              <div className="bg-white rounded-3xl border border-black/[0.06] p-5 shadow-sm">
                <h2 className="font-black text-black mb-4 flex items-center gap-2">
                  <Globe className="w-5 h-5 text-black dark:text-white" /> لغة الموقع / النظام
                </h2>
                <div className="grid grid-cols-3 gap-3">
                  {LANGUAGES.map(lang => (
                    <button key={lang.v} onClick={() => set("siteLanguage", lang.v)}
                      data-testid={`setup-lang-${lang.v}`}
                      className={`border-2 rounded-2xl py-5 px-2 text-center transition-all ${form.siteLanguage === lang.v ? "border-black dark:border-white bg-black/[0.04] dark:bg-white/[0.06] shadow-md" : "border-black/[0.07] hover:border-black/20"}`}>
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => setSection(2)} disabled={!canNext1}
                  className="flex-1 h-12 rounded-2xl bg-black dark:bg-white hover:bg-black dark:bg-white text-white font-bold gap-2" data-testid="setup-btn-next-1">
                  التالي <ChevronLeft className="w-4 h-4" />
                </Button>
              </div>
              <button onClick={() => navigate("/dashboard")} className="w-full text-center text-xs text-black/30 hover:text-black/50 py-2 transition-colors">
            ...
            <motion.div key="s2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
              <div className="bg-white rounded-3xl border border-black/[0.06] p-5 shadow-sm">
                <h2 className="font-black text-black mb-1 flex items-center gap-2">
                  <Zap className="w-5 h-5 text-black dark:text-white" /> التكاملات المطلوبة
                </h2>
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => setSection(3)}
                  className="flex-1 h-12 rounded-2xl bg-black dark:bg-white hover:bg-black dark:bg-white text-white font-bold gap-2" data-testid="setup-btn-next-2">
                  التالي <ChevronLeft className="w-4 h-4" />
                </Button>
              </div>
              <button onClick={() => navigate("/dashboard")} className="w-full text-center text-xs text-black/30 hover:text-black/50 py-2 transition-colors">
          ...
          {section === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
              <div className="bg-white rounded-3xl border border-black/[0.06] p-5 shadow-sm">
                <h2 className="font-black text-black mb-4 flex items-center gap-2">
                  <Layers className="w-5 h-5 text-black dark:text-white" /> فكرتك ومتطلباتك
                </h2>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-black/[0.04] dark:bg-white/[0.06] hover:bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white text-[11px] font-bold rounded-lg transition-colors"
              ...
              {/* Summary card */}
              <div className="bg-black/[0.04] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 rounded-2xl p-4 space-y-2">
                <p className="text-xs font-bold text-black dark:text-white mb-2">ملخص ما ستُرسله للفريق:</p>
                {[
                  ...
                  <div key={r.label} className="flex items-start gap-2 text-xs">
                    <span className="text-black dark:text-white font-bold shrink-0 w-16">{r.label}:</span>
                    <span className="text-black dark:text-white font-medium">{r.val}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}
                  className="flex-1 h-12 rounded-2xl bg-gradient-to-r from-black dark:from-white to-black dark:to-white text-white font-black gap-2" data-testid="setup-btn-save">
