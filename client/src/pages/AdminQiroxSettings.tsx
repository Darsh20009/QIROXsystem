...
type Section = "company" | "contact" | "social" | "financial" | "distribution" | "appdownload" | "tracking" | "system";
...
export default function AdminQiroxSettings() {
  ...
  const SECTIONS: { id: Section; label: string; icon: any; color: string }[] = [
    ...
    { id: "system", label: L ? "النظام" : "System", icon: Settings2, color: "text-red-500" },
  ];
    ...
    <div className="relative overflow-hidden min-h-screen bg-white dark:bg-gray-950 p-6" dir={dir}>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <motion.div key={section} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          {/* Terms & Conditions */}
          {section === "tracking" && (
            <div className="mt-5 border border-black/[0.07] dark:border-white/[0.07] rounded-2xl p-6 space-y-5 bg-white dark:bg-gray-900">
              <h3 className="font-bold text-black dark:text-white flex items-center gap-2">
                <FileText className="w-4 h-4" /> {L ? "الشروط والأحكام" : "Terms & Conditions"}
              </h3>
              <div>
                <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-1.5 block">{L ? "الشروط العامة للشركة" : "Company General Terms"}</label>
                <Textarea
                  value={(form as any).termsAndConditions || ""}
                  onChange={e => set("termsAndConditions" as any, e.target.value)}
                  placeholder={L ? "الشروط والأحكام العامة تظهر في العقود والفواتير..." : "General terms & conditions shown in contracts and invoices..."}
                  rows={5}
                  data-testid="textarea-terms"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-1.5 block">{L ? "شروط عروض الأسعار" : "Quotation Terms"}</label>
                <Textarea
                  value={(form as any).quotationTerms || ""}
                  onChange={e => set("quotationTerms" as any, e.target.value)}
                  placeholder={L ? "تظهر أسفل عروض الأسعار تلقائياً..." : "Appears at the bottom of quotations automatically..."}
                  rows={4}
                  data-testid="textarea-quotation-terms"
                />
              </div>
            </div>
          )}
          ...
          {section === "system" && (
            <div className="space-y-4">
              <div className="border border-red-200/60 dark:border-red-800/30 rounded-2xl p-6 space-y-5 bg-white dark:bg-gray-900">
                ...
                <p className="text-xs text-black/40 dark:text-white/40">
                  {L ? "عند التفعيل، تظهر رسالة صيانة لجميع الزوار وتُوقف صفحات الدخول. الـ Admin لا يتأثر." : "When active, a maintenance message is shown to all visitors. Admins are unaffected."}
                </p>
                <label className="flex items-center gap-4 cursor-pointer select-none">
                  <div
                    onClick={() => set("maintenanceMode" as any, !(form as any).maintenanceMode)}
                    className={`relative w-12 h-6 rounded-full transition-colors duration-200 flex-shrink-0 ${(form as any).maintenanceMode ? "bg-red-500" : "bg-black/15 dark:bg-white/15"}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${(form as any).maintenanceMode ? "right-1" : "left-1"}`} />
                  </div>
                  <span className={`font-bold text-sm ${(form as any).maintenanceMode ? "text-red-500" : "text-black/50 dark:text-white/40"}`}>
                    {(form as any).maintenanceMode ? (L ? "⚠️ وضع الصيانة مفعّل" : "⚠️ Maintenance Mode ON") : (L ? "وضع الصيانة معطّل" : "Maintenance Mode OFF")}
                  </span>
                </label>
                <div>
                  <label className="label-xs">{L ? "رسالة الصيانة (عربي)" : "Maintenance Message (AR)"}</label>
                  <Textarea value={(form as any).maintenanceMsgAr || ""} onChange={e => set("maintenanceMsgAr" as any, e.target.value)} rows={2} placeholder="الموقع قيد الصيانة، سنعود قريباً..." />
                </div>
                <div>
                  <label className="label-xs">{L ? "رسالة الصيانة (إنجليزي)" : "Maintenance Message (EN)"}</label>
                  <Textarea value={(form as any).maintenanceMsgEn || ""} onChange={e => set("maintenanceMsgEn" as any, e.target.value)} rows={2} placeholder="We're under maintenance, we'll be back soon..." />
                </div>
              </div>
            </div>
          )}
