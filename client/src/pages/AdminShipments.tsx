...
export default function AdminShipments() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false);
      setForm({ ...emptyForm });
      toast({ title: L ? "تم إنشاء الشحنة" : "Shipment created" });
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      setViewItem(updated);
      setUpdateNote("");
      toast({ title: L ? "تم تحديث حالة الشحنة" : "Shipment status updated" });
    },
  });
  ...
  const handleCreate = () => {
    if (!form.productName || !form.clientName || !form.clientEmail) {
      return toast({ title: L ? "اسم المنتج والعميل مطلوبان" : "Product name and client are required", variant: "destructive" });
    }
  ...
  return (
    <div dir={dir}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-black text-black dark:text-white">{L ? "شحنات الأجهزة" : "Hardware Shipments"}</h1>
          <p className="text-sm text-black/40 dark:text-white/40 mt-1">{L ? "تتبع وإدارة شحنات المنتجات والأجهزة" : "Track and manage product & hardware shipments"}</p>
        </div>
        <Button onClick={() => { setEditingId(null); setForm({ ...emptyForm }); setDialogOpen(true); }}
          ...
          <Plus className="w-4 h-4" />{L ? "شحنة جديدة" : "New Shipment"}
        </Button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder={L ? "بحث بالاسم أو الكود..." : "Search by name or code..."} className="pr-9 max-w-xs" data-testid="input-search-shipments" />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-black/30 dark:text-white/30">
          <Truck className="w-10 h-10 mx-auto mb-3 opacity-70" />
          <p>{L ? "لا توجد شحنات" : "No shipments found"}</p>
        </div>
      ) : (
            ...
            return (
              <motion.div key={s.id || s._id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-black/40 dark:text-white/40">{L ? "العميل:" : "Client:"} {s.clientName} — {s.clientEmail}</p>
                  <div className="flex items-center gap-3 mt-1 text-xs text-black/40 dark:text-white/40 flex-wrap">
                    {s.courierName && <span>{L ? "شركة الشحن:" : "Courier:"} {s.courierName}</span>}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" dir={dir}>
          <DialogHeader>
            <DialogTitle className="text-right">{L ? "شحنة جديدة" : "New Shipment"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-1 block">{L ? "اسم المنتج *" : "Product Name *"}</label>
              <Input value={form.productName} onChange={e => setForm(f => ({ ...f, productName: e.target.value }))} placeholder={L ? "اسم المنتج" : "Product name"} data-testid="input-product-name" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-1 block">{L ? "الكمية" : "Quantity"}</label>
                <Input type="number" min="1" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-1 block">{L ? "السعر الإجمالي" : "Total Price"}</label>
            ...
            <div className="border-t border-black/[0.06] dark:border-white/[0.06] pt-3">
              <p className="text-xs font-semibold text-black/60 dark:text-white/60 mb-2">{L ? "بيانات العميل" : "Client Info"}</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "الاسم *" : "Name *"}</label>
                  <Input value={form.clientName} onChange={e => setForm(f => ({ ...f, clientName: e.target.value }))} data-testid="input-shipment-client-name" />
                </div>
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "البريد *" : "Email *"}</label>
                ...
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "الهاتف" : "Phone"}</label>
                  <Input value={form.clientPhone} onChange={e => setForm(f => ({ ...f, clientPhone: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="border-t border-black/[0.06] dark:border-white/[0.06] pt-3">
              <p className="text-xs font-semibold text-black/60 dark:text-white/60 mb-2">{L ? "عنوان الشحن" : "Shipping Address"}</p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { key: "name", label: L ? "الاسم الكامل" : "Full Name" },
                  { key: "phone", label: L ? "الهاتف" : "Phone" },
                  { key: "city", label: L ? "المدينة" : "City" },
                  { key: "district", label: L ? "الحي" : "District" },
                  { key: "street", label: L ? "الشارع" : "Street" },
                  { key: "postalCode", label: L ? "الرمز البريدي" : "Postal Code" },
                ].map(field => (
                  <div key={field.key}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={form.courierName} onChange={e => setForm(f => ({ ...f, courierName: e.target.value }))} placeholder={L ? "أرامكس، DHL..." : "Aramex, DHL..."} />
              </div>
              <div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button onClick={handleCreate} disabled={createMutation.isPending} className="flex-1 bg-black dark:bg-white text-white dark:text-black rounded-xl" data-testid="button-create-shipment">
                {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : (L ? "إنشاء الشحنة" : "Create Shipment")}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!viewItem} onOpenChange={v => !v && setViewItem(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" dir={dir}>
          <DialogHeader>
            <DialogTitle className="text-right">{L ? "تفاصيل الشحنة" : "Shipment Details"}</DialogTitle>
          </DialogHeader>
          {viewItem && (
            <div className="space-y-4">
              <div className="bg-black/[0.03] dark:bg-white/[0.04] rounded-xl p-4">
                ...
                <p className="text-xs text-black/50 dark:text-white/50">{L ? "العميل:" : "Client:"} {viewItem.clientName} — {viewItem.clientEmail}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "شركة الشحن" : "Courier Company"}</label>
                  <Input value={viewItem.courierName || ""} onChange={e => setViewItem((s: any) => ({ ...s, courierName: e.target.value }))} placeholder={L ? "أرامكس، DHL..." : "Aramex, DHL..."} />
                </div>
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "رقم التتبع" : "Tracking Number"}</label>
                  <Input value={viewItem.trackingNumber || ""} onChange={e => setViewItem((s: any) => ({ ...s, trackingNumber: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "رابط التتبع" : "Tracking URL"}</label>
                  <Input value={viewItem.courierUrl || ""} onChange={e => setViewItem((s: any) => ({ ...s, courierUrl: e.target.value }))} placeholder="https://..." />
                </div>
                <div>
                  <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "الوصول المتوقع" : "Estimated Delivery"}</label>
              ...
              <div>
                <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-2 block">{L ? "تحديث الحالة" : "Update Status"}</label>
                ...
                <Input value={updateNote} onChange={e => setUpdateNote(e.target.value)} placeholder={L ? "ملاحظة (اختياري)..." : "Note (optional)..."} className="mt-2" data-testid="input-status-note" />
              </div>

              {viewItem.statusHistory?.length > 0 && (
                <div>
                  <label className="text-xs font-semibold text-black/60 dark:text-white/60 mb-2 block">{L ? "سجل التحديثات" : "Update History"}</label>
                    ...
                    {[...viewItem.statusHistory].reverse().map((h: any, i: number) => (
                      <div key={i} className="flex items-start gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p className="text-black/30 dark:text-white/30">{new Date(h.timestamp).toLocaleString(L ? 'ar-SA' : 'en-US')}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                <label className="text-xs text-black/50 dark:text-white/50 mb-1 block">{L ? "ملاحظات داخلية" : "Internal Notes"}</label>
                <Textarea value={viewItem.adminNotes || ""} onChange={e => setViewItem((s: any) => ({ ...s, adminNotes: e.target.value }))} rows={2} />
              </div>

              <div className="flex gap-2 pt-2">
                <Button variant="outline" onClick={() => setViewItem(null)} className="flex-1 rounded-xl">{L ? "إغلاق" : "Close"}</Button>
                <Button onClick={handleUpdateStatus} disabled={updateMutation.isPending} className="flex-1 bg-black dark:bg-white text-white dark:text-black rounded-xl" data-testid="button-update-shipment">
                  {updateMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : (L ? "تحديث" : "Update")}
                </Button>
              </div>
            </div>
          )}
