...
export default function AdminReferrals() {
  ...
  const L = lang === "ar";
  const statusLabel: Record<string, string> = L ? { pending: "معلّق", rewarded: "تمت المكافأة", expired: "منتهي" } : { pending: "Pending", rewarded: "Rewarded", expired: "Expired" };
    ...
    <div className="p-4 space-y-5" dir={dir}>
      <div>
        <h1 className="text-xl font-black text-black dark:text-white flex items-center gap-2">
          ...
          {L ? "سجل الإحالات" : "Referrals Log"}
        </h1>
        <p className="text-xs text-black/40 dark:text-white/40 mt-0.5">{L ? "إجمالي الإحالات والمكافآت في النظام" : "Total referrals and rewards in the system"}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-2xl border border-emerald-200 dark:border-emerald-800/40 bg-emerald-50 dark:bg-emerald-900/10 p-4 text-center">
          ...
          <p className="text-xs text-black/40 dark:text-white/40 mt-1">{L ? "إحالات ناجحة" : "Successful Referrals"}</p>
        </div>
        <div className="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-amber-50 dark:bg-amber-900/10 p-4 text-center">
          ...
          <p className="text-xs text-black/40 dark:text-white/40 mt-1 flex items-center justify-center gap-1"><SARIcon size={11} /> {L ? "وُزّعت" : "Distributed"}</p>
        </div>
      </div>
