...
export default function AdminReferrals() {
  ...
  const statusColor: Record<string, string> = {
    pending: "bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white text-black dark:text-white dark:text-black/70 dark:text-white/70",
    rewarded: "bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white text-black dark:text-white dark:text-black/70 dark:text-white/70",
  ...
  return (
    <div className="p-4 space-y-5" dir={dir}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-2xl border border-black/10 dark:border-white/10 dark:border-black dark:border-white bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white p-4 text-center">
          <div className="w-8 h-8 bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white rounded-xl flex items-center justify-center mx-auto mb-2">
            <Award className="w-4 h-4 text-black dark:text-white dark:text-black/70 dark:text-white/70" />
          </div>
          <p className="text-2xl font-black text-black dark:text-white dark:text-black/70 dark:text-white/70">{totalRewarded}</p>
          <p className="text-xs text-black/40 dark:text-white/40 mt-1">{L ? "إحالات ناجحة" : "Rewarded"}</p>
        </div>
        <div className="rounded-2xl border border-black/10 dark:border-white/10 dark:border-black dark:border-white bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white p-4 text-center">
          <div className="w-8 h-8 bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white rounded-xl flex items-center justify-center mx-auto mb-2">
            <Clock className="w-4 h-4 text-black dark:text-white dark:text-black/70 dark:text-white/70" />
          </div>
          <p className="text-2xl font-black text-black dark:text-white dark:text-black/70 dark:text-white/70">{totalPending}</p>
          <p className="text-xs text-black/40 dark:text-white/40 mt-1">{L ? "معلّقة" : "Pending"}</p>
        </div>
        <div className="rounded-2xl border border-black/10 dark:border-white/10 dark:border-black dark:border-white bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white p-4 text-center">
          <div className="w-8 h-8 bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white rounded-xl flex items-center justify-center mx-auto mb-2">
            <TrendingUp className="w-4 h-4 text-black dark:text-white dark:text-black/70 dark:text-white/70" />
          </div>
          <p className="text-2xl font-black text-black dark:text-white dark:text-black/70 dark:text-white/70">{conversionRate}%</p>
          <p className="text-xs text-black/40 dark:text-white/40 mt-1">{L ? "معدل التحويل" : "Conversion Rate"}</p>
        </div>
      </div>

      {/* Credits distributed */}
      <div className="rounded-2xl border border-black/10 dark:border-white/10 dark:border-black dark:border-white bg-gradient-to-r from-black/[0.04] dark:from-white/[0.06] to-black/[0.04] dark:to-white/[0.06] dark:from-black dark:from-white dark:to-black dark:to-white p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-black dark:text-white dark:text-black/70 dark:text-white/70 font-medium">{L ? "إجمالي المكافآت الموزّعة" : "Total Credits Distributed"}</p>
            <div className="flex items-center gap-1 mt-1">
              <p className="text-3xl font-black text-black dark:text-white dark:text-black/70 dark:text-white/70">{totalCredits.toLocaleString()}</p>
              <SARIcon size={18} className="text-black dark:text-white dark:text-black/70 dark:text-white/70 mt-1" />
            </div>
          </div>
          <div className="w-12 h-12 bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white rounded-2xl flex items-center justify-center">
            <BarChart2 className="w-6 h-6 text-black dark:text-white dark:text-black/70 dark:text-white/70" />
          </div>
        </div>
        {totalRewarded > 0 && (
          <p className="text-xs text-black dark:text-white dark:text-black/70 dark:text-white/70 mt-2">
            {L
                ...
                <div key={i} className="flex items-center justify-between py-2 border-b border-black/5 dark:border-white/5 last:border-0">
                  <div className="flex items-center gap-2">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black ${
                      i === 0 ? "bg-black/[0.04] dark:bg-white/[0.06] text-black dark:text-white" : i === 1 ? "bg-gray-100 text-gray-500" : "bg-black/5 text-black/40"
                    }`}>{i + 1}</div>
                    <span className="text-sm font-medium text-black dark:text-white">{ref.name}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-bold text-black dark:text-white dark:text-black/70 dark:text-white/70">{ref.count} {L ? "إحالة" : "refs"}</span>
          ...
          <Card className="border-black/10 dark:border-white/10 dark:bg-gray-900">
            ...
            <CardContent className="space-y-3">
              {[
                ...
                { label: L ? "معلّقة" : "Pending", count: totalPending, color: "bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white" },
                { label: L ? "ناجحة" : "Rewarded", count: totalRewarded, color: "bg-black/[0.04] dark:bg-white/[0.06] dark:bg-black dark:bg-white" },
              ...
              <tbody className="divide-y divide-black/[0.03] dark:divide-white/[0.03]">
                {referrals.map((r: any) => (
                  <tr key={r._id} className="hover:bg-black/[0.015] dark:hover:bg-white/[0.015]" data-testid={`row-admin-referral-${r._id}`}>
                    ...
                    <td className="px-4 py-3 font-bold text-black dark:text-white dark:text-black/70 dark:text-white/70"><span className="flex items-center gap-1">{r.creditAmount} <SARIcon size={11} /></span></td>
