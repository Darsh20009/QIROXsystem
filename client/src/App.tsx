...
import QiroxLoadingScreen from "@/components/QiroxLoadingScreen";
...
const AdminProjectRenewals = lazy(() => import("@/pages/AdminProjectRenewals"));
...
const AdminStoreWhatsApp = lazy(() => import("@/pages/AdminStoreWhatsApp"));
...
const ADMIN_MANAGER_HR = ["admin", "manager", "hr", "data_entry"] as const;
...
function G_Partners(){ return <RoleGuard allowedRoles={["admin", "manager", "data_entry"]}><AdminPartners /></RoleGuard>; }
function G_ApiKeys(){ return <RoleGuard allowedRoles={["admin", "manager"]}><AdminApiKeys /></RoleGuard>; }
function G_Kanban(){ return <RoleGuard allowedRoles={["admin", "manager", "data_entry"]}><AdminKanban /></RoleGuard>; }
...
function PublicRouter() {
  return (
    <Suspense fallback={<QiroxLoadingScreen />}>
      <Switch>
        ...
        {/* Public support page — must be accessible without login (Apple Guideline 1.5) */}
  ...
  return (
    <Suspense fallback={<QiroxLoadingScreen />}>
      <Switch>
        ...
        <Route path="/admin/partners" component={G_Partners} />
        ...
        <Route path="/admin/project-renewals" component={AdminProjectRenewals} />
        <Route path="/admin/mod-requests" component={AdminModRequests} />
        <Route path="/admin/whatsapp" component={() => <RoleGuard allowedRoles={[...ADMIN_MANAGER]}><AdminWhatsApp /></RoleGuard>} />
        ...
        {/* Keep the shorter navigation URL as a backwards-compatible alias. */}
        <Route path="/admin/activity" component={G_ActivityLog} />
        ...
        <Route path="/admin/api-keys" component={G_ApiKeys} />
        <Route path="/admin/store-whatsapp" component={AdminStoreWhatsApp} />
        ...
        <Route path="/admin/kanban" component={G_Kanban} />
...
const ALL_PAGES = [
  ...
  { title: "طلبات تجديد المشاريع", titleEn: "Project Renewal Requests", url: "/admin/project-renewals", group: "employee" },
...
class PageErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean; retried: boolean }> {
  constructor(props: { children: ReactNode }) {
    super(props);
    // Track recovery by route so an earlier, unrelated page error cannot make
    // a future page immediately fall through to the maintenance screen.
    const retriedAt = Number(sessionStorage.getItem(this.recoveryKey()) || 0);
    const recentRetry = retriedAt > 0 && Date.now() - retriedAt < 2 * 60 * 1000;
    this.state = { hasError: false, retried: recentRetry };
  }

  private recoveryKey() {
    return `__qiroxPageErrorRetry:${window.location.pathname}`;
  }
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    ...
    if (!this.state.retried) {
      // A stale deployment can leave the browser with an old entry bundle that
      // requests a hashed lazy chunk no longer on the server. Clear PWA caches
      // before the one recovery reload so the fresh entry bundle is guaranteed.
      sessionStorage.setItem(this.recoveryKey(), String(Date.now()));
        ...
        void (async () => {
          try {
            ...
            if (window.caches) {
              ...
              await Promise.all(
                keys
                  .filter(key => key.startsWith("qirox-") && !key.startsWith("qirox-pending"))
                  .map(key => caches.delete(key)),
              );
            }
          } catch (recoveryError) {
            console.warn("[QIROX] Page recovery cache reset failed:", recoveryError);
          }
          window.location.reload();
        })();
      });
    } else {
      // A second crash on the same route should never expose the maintenance
      // screen. Return the user to the public landing page so a broken lazy
      // route cannot make the whole application look offline.
  ...
  render() {
    if (this.state.hasError) {
      // First error: silently reloading — don't flash an error page.
      if (!this.state.retried) return null;
      // If the retry also failed, keep a neutral loading shell while the
      // recovery navigation above completes. Never render the old maintenance
      // page here.
      return <QiroxLoadingScreen />;

      /*
       * Legacy maintenance fallback intentionally kept unreachable for now so
       * the existing recovery markup can be removed safely in a later cleanup.
       */
      return (
        <div dir="rtl" style={{
          ...
          <div style={{
            ...
            <div style={{ maxWidth: 780, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: "2rem" }}>
              ...
              {/* ── Lightweight system graphic ── */}
              <div style={{
                width: "100%", maxWidth: 620, borderRadius: 18, overflow: "hidden",
                border: "1px solid rgba(255,255,255,0.08)",
                background: "linear-gradient(135deg, #15151c 0%, #101116 100%)", position: "relative",
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}>
                        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.72)" strokeWidth="1.8">
                          <path d="M12 3v18M3 12h18M5.6 5.6l12.8 12.8M18.4 5.6 5.6 18.4" />
                          <circle cx="12" cy="12" r="3.2" fill="rgba(255,255,255,0.18)" />
                        </svg>
                      </div>
                      <div>
                        <div style={{ color: "rgba(255,255,255,0.76)", fontSize: "0.78rem", fontWeight: 800 }}>QIROX Systems</div>
                        <div style={{ color: "rgba(255,255,255,0.28)", fontSize: "0.62rem", marginTop: 2 }}>نعيد ترتيب التجربة</div>
                      </div>
                    </div>
                    <span style={{ color: "rgba(255,255,255,0.38)", fontSize: "0.6rem", fontFamily: "monospace", letterSpacing: "0.12em" }}>SYNCING</span>
                  </div>
                  <svg width="100%" viewBox="0 0 620 188" role="img" aria-label="رسم توضيحي لتحديث النظام" style={{ display: "block" }}>
                    <defs>
                      <linearGradient id="qirox-recovery-line" x1="0" y1="0" x2="1" y2="0">
                        <stop offset="0%" stopColor="rgba(255,255,255,0.08)" />
                        <stop offset="50%" stopColor="rgba(255,255,255,0.5)" />
                        <stop offset="100%" stopColor="rgba(255,255,255,0.08)" />
                      </linearGradient>
                    </defs>
                    <path d="M92 94H238M382 94H528" stroke="url(#qirox-recovery-line)" strokeWidth="2" />
                    <path d="M165 52v84M455 52v84" stroke="rgba(255,255,255,0.08)" strokeDasharray="3 7" />
                    <rect x="30" y="58" width="124" height="72" rx="14" fill="rgba(255,255,255,0.035)" stroke="rgba(255,255,255,0.11)" />
                    <rect x="248" y="34" width="124" height="120" rx="18" fill="rgba(255,255,255,0.07)" stroke="rgba(255,255,255,0.22)" />
                    <rect x="466" y="58" width="124" height="72" rx="14" fill="rgba(255,255,255,0.035)" stroke="rgba(255,255,255,0.11)" />
                    <circle cx="92" cy="94" r="14" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.28)" />
                    <circle cx="310" cy="94" r="26" fill="rgba(255,255,255,0.1)" stroke="rgba(255,255,255,0.48)" />
                    <circle cx="528" cy="94" r="14" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.28)" />
                    <path d="m302 94 6 6 12-14" fill="none" stroke="rgba(255,255,255,0.82)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
                    <text x="92" y="119" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="11" fontFamily="Arial">واجهة</text>
                    <text x="310" y="137" textAnchor="middle" fill="rgba(255,255,255,0.72)" fontSize="12" fontFamily="Arial" fontWeight="700">QIROX</text>
                    <text x="528" y="119" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="11" fontFamily="Arial">خدمات</text>
                  </svg>
                </div>
              </div>
