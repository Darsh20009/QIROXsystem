...
export type InboxSocketEvent =
  ...
  | { type: "task_shared"; taskId: string; taskTitle: string; sharedBy: string }
...
export function useInboxSocket({ userId, onEvent }: UseInboxSocketOptions) {
  ...
  const connect = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      const attempt = (wsRef.current as any)?._retryAttempt ?? 0;
      const delay = Math.min(3000 * Math.pow(2, attempt), 30000);
      reconnectTimerRef.current = setTimeout(() => {
        const newWs = wsRef.current;
        if (newWs) (newWs as any)._retryAttempt = attempt + 1;
        connect();
      }, delay);
    };
