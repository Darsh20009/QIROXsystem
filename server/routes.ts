...
async function atomicWalletDebit(userId: string, amount: number): Promise<boolean> {
  ...
  const result = await UserModel.findOneAndUpdate(
    { _id: userId, walletBalance: { $gte: parseFloat((amount - 0.001).toFixed(4)) } },
...
export async function registerRoutes(
  ...
  app.get("/api/health", async (_req, res) => {
    try {
      const { mongoose } = await import("mongoose").catch(() => ({ mongoose: null }));
    ...
    try {
      ...
      req.login(user, async (err) => {
        ...
        if (user.email) {
          ...
          // Notify managers from DB
          UserModel.find({ role: { $in: ["admin", "manager"] }, email: { $exists: true, $ne: "" } }).select("email fullName").lean().then((mgrs: any[]) => {
            for (const m of mgrs) {
              if (m.email && m.email !== "info@qiroxstudio.online" && m.email !== "qiroxsystem@gmail.com") {
                sendAdminNewClientEmail(m.email, user.fullName || user.username, user.email, (user as any).phone || "", "التسجيل الذاتي").catch(console.error);
              }
            }
          }).catch(console.error);
          sendOwnerWAEmail({
  ...
  const userFieldsWhitelist = ["username", "password", "email", "fullName", "role", "phone", "avatarUrl", "instagram", "twitter", "linkedin", "snapchat", "tiktok", "youtube", "linktree", "jobTitle", "workEmail", "bio", "address", "city", "taxNumber", "organizationName", "commercialRegistration", "nationalAddress", "allowedPages"];
  ...
  app.post("/api/admin/users", async (req, res) => {
    ...
    try {
      const { username, password, email, fullName, role, phone, createWorkEmail, workEmailAddress } = req.body;
      ...
      if (createWorkEmail && workEmailAddress) {
        ...
        const normalizedEmail = String(workEmailAddress).toLowerCase().trim();
        try {
          // 1) Create real cPanel email account
          const { cpanelCreateEmail } = await import("./cpanel");
          const emailPassword = password; // use the same password the employee was created with
          const cpResult = await cpanelCreateEmail(normalizedEmail, emailPassword);
          if (cpResult.alreadyExists) {
            console.log(`[cPanel] Email already exists: ${normalizedEmail}`);
          } else {
            console.log(`[cPanel] Email created: ${normalizedEmail}`);
          }
          // 2) Save MongoDB MailAccount record
          await MAM.findOneAndUpdate(
            { emailAddress: normalizedEmail },
            {
              ...
              smtpHost: smtpHost,
              smtpPort: smtpPort,
              assignedUserId: (user as any)._id || (user as any).id,
              assignedUserIds: [(user as any)._id || (user as any).id],
              isShared: false,
              sharedWith: [],
            },
            { upsert: true, new: true }
          );
          console.log(`[Mail] Work email record saved: ${normalizedEmail} → user ${user.username}`);
        } catch (mailErr: any) {
          console.error("[Mail] Failed to create work email:", mailErr.message);
        }
      }
  ...
  // Seed 4 system mail accounts (hr, marketing, info, support) — creates real cPanel inboxes + MongoDB records
  app.post("/api/admin/seed-system-mail-accounts", async (req, res) => {
    if (!req.isAuthenticated() || !["admin","manager"].includes((req.user as any).role)) return res.sendStatus(403);
    try {
      const { MailAccountModel: MAM2 } = await import("./models");
      const { cpanelCreateEmail } = await import("./cpanel");
      const smtpHost = process.env.SMTP_HOST || "server222.web-hosting.com";
      const smtpPass = process.env.SMTP_PASS || "";
      const smtpPort = parseInt(process.env.SMTP_PORT || "465");
      const MGMT = ["admin", "cto", "ceo", "manager"];

      const systemAccounts = [
        { emailAddress: "hr@qirox.online",        displayName: "Qirox HR",        jobTitle: "الموارد البشرية" },
        { emailAddress: "marketing@qirox.online",  displayName: "Qirox Marketing", jobTitle: "التسويق" },
        { emailAddress: "info@qirox.online",       displayName: "Qirox Info",      jobTitle: "المعلومات العامة" },
        { emailAddress: "support@qirox.online",    displayName: "Qirox Support",   jobTitle: "الدعم الفني" },
      ];

      const results = [];
      for (const acc of systemAccounts) {
        // 1) Create real cPanel inbox (silently skip if already exists)
        let cpanelStatus = "created";
        try {
          const cpResult = await cpanelCreateEmail(acc.emailAddress, smtpPass);
          cpanelStatus = cpResult.alreadyExists ? "already_exists" : "created";
          console.log(`[cPanel] ${acc.emailAddress}: ${cpanelStatus}`);
        } catch (cpErr: any) {
          cpanelStatus = `error: ${cpErr.message}`;
          console.error(`[cPanel] Failed to create ${acc.emailAddress}:`, cpErr.message);
        }

        // 2) Upsert MongoDB MailAccount record
        const doc = await MAM2.findOneAndUpdate(
        ...
        results.push({ emailAddress: acc.emailAddress, id: doc._id.toString(), cpanelStatus });
      }
      console.log("[Mail] System accounts seeded:", results.map(r => r.emailAddress).join(", "));
  ...
  // === PROVISION EMPLOYEE EMAILS (cPanel) ===
  app.post("/api/admin/provision-employee-emails", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role !== "admin") return res.sendStatus(403);
    try {
      const days = Number(req.body.days) || 30;
      const { provisionEmployeeEmails } = await import("./cpanel");
      const result = await provisionEmployeeEmails(days);
  ...
  app.post("/api/contact", contactLimiter, async (req, res) => {
    try {
      ...
      // trim first, then validate — prevents whitespace-only submissions passing
      const tName = String(name || "").trim();
      const tEmail = String(email || "").trim();
      const tMessage = String(message || "").trim();
      if (!tName || !tEmail || !tMessage) return res.status(400).json({ error: "يرجى تعبئة جميع الحقول المطلوبة" });
      const safeName = escapeHtml(tName.slice(0, 200));
      const safeEmail = escapeHtml(tEmail.slice(0, 200));
      const safePhone = escapeHtml(String(phone || "").trim().slice(0, 20));
      const safeSubject = escapeHtml(String(subject || "").trim().slice(0, 300));
      const safeMessage = escapeHtml(tMessage.slice(0, 5000));
  ...
  app.get("/api/admin/orders", async (req, res) => {
    ...
    const currentUserId = String((req.user as any).id || (req.user as any)._id);
    ...
    const filter: any = {};
    // Regular employees only see orders assigned to them
  ...
  app.post("/api/orders", async (req, res) => {
    ...
    // Notify all managers + sales staff from DB
    UserModel.find({ role: { $in: ["admin", "manager", "supervisor"] }, email: { $exists: true, $ne: "" } }).select("email fullName").lean().then((staffList: any[]) => {
      for (const s of staffList) {
        if (s.email && s.email !== "info@qiroxstudio.online" && s.email !== "qiroxsystem@gmail.com") {
          sendAdminNewOrderEmail(s.email, _clientName, _clientEmail, String(order.id), items, req.body.totalAmount).catch(console.error);
        }
      }
    }).catch(console.error);
    sendOwnerWAEmail({
...
<body>
    ...
    try {
      const result = await cache.getOrFetch("admin:stats", async () => {
        ...
        const todayRevenue = allOrders
          .filter(o => new Date(o.createdAt) >= todayStart && (o.status === "completed" || o.status === "approved" || o.status === "in_progress"))
  ...
  app.delete("/api/notifications", async (req, res) => {
    ...
    const isStaff = ["admin", "manager", "developer", "designer", "support", "sales_manager", "sales", "accountant", "merchant"].includes(role);
    const baseQuery = isStaff ? { $or: [{ userId: uid }, { forAdmins: true }] } : { userId: uid };
    const query = onlyRead ? { ...baseQuery, read: true } : baseQuery;
  ...
  app.post("/api/inbox", async (req, res) => {
    ...
    try {
      ...
      // Send email notification to all users (clients AND employees) for new messages
  ...
  app.post("/api/inbox/call/ring", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { targetUserId, roomId, callType = "video" } = req.body;
      if (!targetUserId || !roomId) return res.status(400).json({ error: "targetUserId and roomId required" });
      const { UserModel } = await import("./models");
      const caller = req.user as any;
      const callerName = caller.fullName || caller.username;
      const callerPhoto = caller.profilePhotoUrl || "";
      const callerRole = caller.role || "";
      const callerId = String(caller.id || caller._id);
      // Send ring event to target
      const { pushToUser } = await import("./ws");
      pushToUser(String(targetUserId), {
        type: "call_ring",
        callerId,
        callerName,
        callerPhoto,
      ...
      // Also notify via in-app notification (lower priority)
      fireNotify(
        String(targetUserId),
        `📞 مكالمة واردة من ${callerName}`,
        callType === "video" ? "مكالمة فيديو — افتح الإشعار للرد" : "مكالمة صوتية — افتح الإشعار للرد",
        { type: "call", link: "/inbox", icon: "📞", tag: `call-${callerId}` }
      ).catch(() => {});
      res.json({ ok: true, callerId, callerName, roomId });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });

  app.post("/api/inbox/call/accept", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { callerId, roomId } = req.body;
      const { pushToUser } = await import("./ws");
      const me = req.user as any;
      pushToUser(String(callerId), {
        type: "call_accepted",
        acceptedBy: String(me.id || me._id),
        acceptedName: me.fullName || me.username,
        roomId,
      });
      res.json({ ok: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  ...
  app.post("/api/inbox/call/decline", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { callerId, roomId } = req.body;
      const { pushToUser } = await import("./ws");
      ...
      pushToUser(String(callerId), {
        type: "call_declined",
        declinedBy: String(me.id || me._id),
        declinedName: me.fullName || me.username,
        roomId,
      });
      res.json({ ok: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  ...
  app.post("/api/inbox/call/missed", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { targetUserId, callerName } = req.body;
      const { UserModel } = await import("./models");
      const target = await (UserModel as any).findById(targetUserId).select("email fullName username").lean();
      if (target?.email) {
        sendIncomingCallEmail(target.email, (target as any).fullName || (target as any).username, callerName).catch(console.error);
      }
      res.json({ ok: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.get("/api/inbox/contacts", async (req, res) => {
    ...
    try {
      ...
      const EMPLOYEE_ROLES = ["admin","manager","ceo","cto","developer","designer","support","sales","sales_manager","accountant","merchant","employee","hr","supervisor","qa","devops","content","seo","marketing"];
  ...
  async function populateSession(session: any) {
    ...
    const [client, agent] = await Promise.all([
      UserModel.findById(obj.clientId).select("username fullName email phone country role profilePhotoUrl avatarConfig").lean(),
  ...
  app.post("/api/cs/sessions", async (req, res) => {
    ...
    // Find available agent (support / admin / manager — fewest active sessions)
    const agents = await UserModel.find({ role: { $in: ['support', 'admin', 'manager'] } }).select("_id").lean();
  ...
  app.post("/api/cs/sessions/:id/messages", async (req, res) => {
    ...
    try {
      ...
      } else if (me.role === 'client' && !session.agentId) {
        // Session is waiting (no agent yet) — broadcast to all agents so they see the new message
        const { broadcastToAll } = await import("./ws");
        const { populateSession } = await import("./cs-utils").catch(() => ({ populateSession: async (s: any) => s }));
        broadcastToAll({ type: 'cs_session_update', action: 'message', sessionId: String(session._id) });
        // Also notify all support/admin/manager agents via push
        const agentUsers = await UserModel.find({ role: { $in: ['support', 'admin', 'manager'] } }).select("_id").lean();
        for (const agent of agentUsers) {
          fireNotify(
            String((agent as any)._id),
            `💬 رسالة انتظار من ${me.fullName || me.username}`,
            notifBody.slice(0, 120),
            { type: "message", link: "/cs-chat", icon: "💬", tag: `cs-waiting-${String(session._id)}` }
          ).catch(() => {});
        }
      }
...
اجعل الفكرة أكثر وضوحاً ودقة من الناحية التقنية وتجارية. لا تضف قائمة أو نقاط — فقرة واحدة متماسكة.`;
  ...
  app.post("/api/admin/test-email", async (req, res) => {
    ...
    else res.status(500).json({ error: "فشل إرسال البريد — تحقق من إعدادات SMTP_HOST وSMTP_USER وSMTP_PASS" });
  });
  ...
  app.get("/api/support-tickets", async (req, res) => {
    ...
    const query: any = isAdmin ? {} : { userId: (req.user as any)._id || (req.user as any).id };
    if (status && status !== "all") query.status = status;
    if (search && isAdmin) query.$or = [
      { subject: { $regex: search, $options: "i" } },
    ];
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const [tickets, total] = await Promise.all([
      (SupportTicketModel as any).find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(parseInt(limit))
        .populate('userId', 'fullName email username'),
      (SupportTicketModel as any).countDocuments(query),
    ]);
  ...
  app.post("/api/support-tickets", async (req, res) => {
    ...
    await logActivity((req.user as any)._id, 'create_ticket', 'support_ticket', ticketId, { subject }, req.ip);
    const user = req.user as any;
    // Notify admins about new ticket (in-app + email)
    try {
      const { NotificationModel, UserModel } = await import("./models");
      await NotificationModel.create({
      ...
      if (user.email) {
        sendSupportTicketCreatedEmail(user.email, user.fullName || user.username, subject, category || 'general', priority || 'medium', ticketId).catch(console.error);
      }
      // Send email to admins, managers, and support staff
      const ticketStaff = await (UserModel as any).find({ role: { $in: ['admin', 'manager', 'supervisor', 'support'] } }).select('email fullName').lean();
      for (const admin of ticketStaff) {
        if (admin.email) {
          sendAdminNewTicketEmail(admin.email, admin.fullName || 'المدير', user.fullName || user.username, subject, body, priority || 'medium', ticketId).catch(console.error);
        }
      }
    } catch (_) {}
    res.json(ticket);
  ...
  app.patch("/api/admin/support-tickets/:id", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    const { SupportTicketModel } = await import("./models");
    const { adminReply, status, assignedTo } = req.body;
    const update: any = {};
    ...
    const ticket = await (SupportTicketModel as any).findByIdAndUpdate(req.params.id, update, { returnDocument: "after" }).populate('userId', 'fullName email username');
    ...
    try {
      if (ticket) {
        ...
        const clientUser = ticket.userId as any;
        const ticketId = ticket._id?.toString() || req.params.id;
        if (adminReply) {
          await NotificationModel.create({
            userId: String(ticket.userId?._id || ticket.userId), type: 'support',
            title: 'رد على تذكرتك',
          ...
          // Send email to user
          if (clientUser?.email) {
            sendSupportTicketReplyEmail(clientUser.email, clientUser.fullName || clientUser.username, ticket.subject, adminReply, status || ticket.status, ticketId).catch(console.error);
          }
        } else if (status) {
          ...
          if (clientUser?.email) {
            sendSupportTicketReplyEmail(clientUser.email, clientUser.fullName || clientUser.username, ticket.subject, `تم تحديث حالة تذكرتك إلى: ${statusLabels[status] || status}`, status, ticketId).catch(console.error);
          }
        }
      }
    } catch (_) {}
  ...
  app.get("/api/employee/apple-wallet-pass", async (req, res) => {
    ...
    const certsDir = pathLib.default.join(process.cwd(), "server", "certs");
      ...
      // Render environment variables sometimes contain escaped newlines and
      // exported Keychain PEMs can include "Bag Attributes" before the key.
      // passkit-generator expects the PEM block itself, not that metadata.
      const normalizePem = (value: string): string => {
        const pem = String(value || "").replace(/\\n/g, "\n").trim();
        const begin = pem.indexOf("-----BEGIN");
        return begin >= 0 ? pem.slice(begin).trim() : pem;
      };
      const passCert   = normalizePem(readCert("APPLE_PASS_CERT", "apple-pass-cert.pem"));
      const passKey    = normalizePem(readCert("APPLE_PASS_KEY",  "apple-pass-key.pem"));
      const wwdr       = normalizePem(readCert("APPLE_WWDR_CERT", "apple-wwdr.pem"));
    ...
    try {
      ...
      try {
        ...
        const passkitModule: any = await _dynImport("passkit-generator");
        // Node's import() returns different shapes for the ESM package and
        // the CommonJS compatibility build. Support both to avoid the
        // minified "… is not a function" failure seen on Render.
        const PKPass = passkitModule?.PKPass
          || passkitModule?.default?.PKPass
          || passkitModule?.default;
        if (typeof PKPass !== "function") {
          throw new Error("passkit-generator PKPass export is unavailable");
        }
        ...
        if (typeof pass?.getAsBuffer !== "function") {
          throw new Error("passkit-generator returned an invalid PKPass instance");
        }
  ...
  app.get("/api/admin/employee-profiles", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","hr"].includes(_role)) return res.sendStatus(403);
    const { EmployeeProfileModel } = await import("./models");
  ...
  app.get("/api/admin/payroll", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","accountant","hr"].includes(_role)) return res.sendStatus(403);
  ...
  // Send payslip email
  app.post("/api/admin/payroll/:id/send-email", async (req, res) => {
    if (!req.isAuthenticated() || !["admin","manager","accountant","hr"].includes((req.user as any).role)) return res.sendStatus(403);
    try {
      const { PayrollRecordModel, UserModel } = await import("./models");
      const rec = await (PayrollRecordModel as any).findById(req.params.id).lean() as any;
      if (!rec) return res.sendStatus(404);
      const emp = await (UserModel as any).findById(rec.userId).lean() as any;
      if (!emp?.email) return res.status(400).json({ error: "No email for employee" });
      const monthName = new Date(rec.year, rec.month - 1, 1).toLocaleString("ar-SA", { month: "long", year: "numeric" });
      const html = `<div dir="rtl" style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:24px">
        <h2 style="color:#111">كشف الراتب — ${monthName}</h2>
        <p>عزيزي ${emp.fullName || emp.username}،</p>
        <table style="width:100%;border-collapse:collapse;margin-top:16px">
          <tr><td style="padding:8px;border-bottom:1px solid #eee">الراتب الأساسي</td><td style="padding:8px;border-bottom:1px solid #eee;font-weight:bold">${rec.baseSalary?.toLocaleString("ar-SA")} ر.س</td></tr>
          <tr><td style="padding:8px;border-bottom:1px solid #eee">المكافآت</td><td style="padding:8px;border-bottom:1px solid #eee;color:#16a34a">${(rec.bonuses || 0).toLocaleString("ar-SA")} ر.س</td></tr>
          <tr><td style="padding:8px;border-bottom:1px solid #eee">الخصومات</td><td style="padding:8px;border-bottom:1px solid #eee;color:#dc2626">${(rec.deductions || 0).toLocaleString("ar-SA")} ر.س</td></tr>
          <tr style="background:#f9f9f9"><td style="padding:8px;font-weight:bold">صافي الراتب</td><td style="padding:8px;font-weight:bold;font-size:18px">${rec.netSalary?.toLocaleString("ar-SA")} ر.س</td></tr>
        </table>
        <p style="color:#666;font-size:12px;margin-top:16px">فريق QIROX Studio</p>
      </div>`;
      const { sendEmail } = await import("./lib/email");
      await sendEmail({ to: emp.email, subject: `كشف راتب ${monthName}`, html });
      res.json({ ok: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.post("/api/employee/leave-request", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    try {
      const { from, to, reason, days } = req.body;
      if (!from || !to || !reason) return res.status(400).json({ error: "from/to/reason required" });
      const { NotificationModel, UserModel } = await import("./models");
      const user = req.user as any;
      const { broadcastToUsers } = await import("./ws");
      // Notify admins/hr/manager
      const admins = await (UserModel as any).find({ role: { $in: ["admin", "manager", "hr"] } }, "_id").lean();
      const adminIds = admins.map((a: any) => String(a._id));
      const notifBody = `${user.fullName || user.username} — ${days} يوم — من ${from} إلى ${to}`;
      await NotificationModel.insertMany(adminIds.map((uid: string) => ({
        userId: uid, type: "info", title: "🌴 طلب إجازة جديد", body: notifBody, link: "/admin/attendance", icon: "🌴", read: false,
      })));
      broadcastToUsers(adminIds, { type: "notification", title: "🌴 طلب إجازة جديد", body: notifBody, link: "/admin/attendance" });
      res.json({ ok: true, days, from, to, reason });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  ...
  // ─── Employee Payments (project-based / manual) ──────────────────────────
  app.get("/api/admin/employee-payments", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","accountant","hr","cto"].includes(_role)) return res.sendStatus(403);
    const { EmployeePaymentModel } = await import("./models");
    const userId = req.query.userId as string | undefined;
    const filter: any = {};
    if (userId) filter.userId = userId;
    const payments = await (EmployeePaymentModel as any).find(filter)
      .sort({ createdAt: -1 })
      .populate('userId', 'fullName email role username')
      .populate('addedBy', 'fullName username');
    res.json(payments);
  ...
  app.post("/api/admin/employee-payments", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","hr","cto"].includes(_role)) return res.sendStatus(403);
    const { EmployeePaymentModel, NotificationModel, UserModel } = await import("./models");
    const { userId, projectName, projectId, amount, type, description, dueDate, notes } = req.body;
    if (!userId || !amount) return res.status(400).json({ error: 'المستخدم والمبلغ مطلوبان' });
    const payment = await (EmployeePaymentModel as any).create({
      userId, projectName: projectName || '', projectId: projectId || undefined,
      amount: Number(amount), type: type || 'salary', description: description || '',
      dueDate: dueDate ? new Date(dueDate) : undefined, notes: notes || '',
      addedBy: (req.user as any)._id, status: 'pending',
    });
    // notify employee
    try {
      const adder = (req.user as any).fullName || (req.user as any).username || 'المسؤول';
      const typeLabels: Record<string, string> = { salary: 'راتب', bonus: 'مكافأة', commission: 'عمولة', allowance: 'بدل', other: 'استحقاق' };
      await (NotificationModel as any).create({
        userId, type: 'finance', title: `${typeLabels[type] || 'استحقاق'} جديد`,
        message: `أضاف ${adder} ${typeLabels[type] || 'استحقاقاً'} بمبلغ ${Number(amount).toLocaleString()} ر.س${projectName ? ` — ${projectName}` : ''}`,
        link: '/employee/my-finance',
      });
      const wss = (global as any).__wss;
      if (wss) {
        wss.clients.forEach((client: any) => {
          if (client.readyState === 1 && client._userId && String(client._userId) === String(userId)) {
            client.send(JSON.stringify({ type: 'notification', message: `لديك ${typeLabels[type] || 'استحقاق'} جديد بمبلغ ${Number(amount).toLocaleString()} ر.س` }));
          }
        });
      }
    } catch (_) {}
  ...
  app.patch("/api/admin/employee-payments/:id", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","accountant","hr","cto"].includes(_role)) return res.sendStatus(403);
    const { EmployeePaymentModel } = await import("./models");
    const payment = await (EmployeePaymentModel as any).findById(req.params.id);
    if (!payment) return res.sendStatus(404);
    const { status, amount, description, notes, dueDate, projectName, type } = req.body;
    if (status !== undefined) { payment.status = status; if (status === 'paid') payment.paidAt = new Date(); }
    ...
    if (projectName !== undefined) payment.projectName = projectName;
    if (type !== undefined) payment.type = type;
    await payment.save();
    res.json(payment);
  });

  app.delete("/api/admin/employee-payments/:id", async (req, res) => {
    const _role = (req.user as any)?.role;
    if (!req.isAuthenticated() || !["admin","manager","hr","cto"].includes(_role)) return res.sendStatus(403);
    const { EmployeePaymentModel } = await import("./models");
    await (EmployeePaymentModel as any).findByIdAndDelete(req.params.id);
    res.json({ ok: true });
  });

  app.get("/api/employee/my-payments", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    const { EmployeePaymentModel } = await import("./models");
    const payments = await (EmployeePaymentModel as any).find({ userId: (req.user as any)._id || (req.user as any).id })
      .sort({ createdAt: -1 })
      .populate('addedBy', 'fullName username');
    res.json(payments);
  });
  ...
  app.post("/api/push/native-token", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: "فشل حفظ token الإشعارات" });
    }
  });

  app.delete("/api/push/native-token", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const { token } = req.body;
    if (!token) return res.status(400).json({ error: "token required" });
    try {
      const { NativePushTokenModel } = await import("./models");
      await NativePushTokenModel.deleteOne({ token });
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: "فشل حذف token الإشعارات" });
    }
  });
  ...
  app.get("/api/checklist", async (req, res) => {
    ...
    try {
      ...
      const items = await (ChecklistItemModel as any)
        .find({ $or: [{ userId: uid }, { assignedTo: uid }, { sharedWith: uid }] })
        .populate("assignedTo", "fullName username")
        .populate("assignedBy", "fullName username")
        .populate("sharedWith", "fullName username")
  ...
  app.post("/api/checklist", async (req, res) => {
    ...
    try {
      ...
      const user = req.user as any;
      const uid = user._id || user.id;
      const { title, description, priority, category, projectId, dueDate, assignedTo, assignNote, coAssignees } = req.body;
      if (!title) return res.status(400).json({ error: "العنوان مطلوب" });
      // coAssignees: array of additional employee IDs stored in sharedWith
      const extraAssignees = Array.isArray(coAssignees) ? coAssignees.filter(Boolean) : [];
      const item = await (ChecklistItemModel as any).create({
        ...
        ...(extraAssignees.length > 0 && { sharedWith: extraAssignees }),
      });
      ...
      for (const coId of extraAssignees) {
        try {
          ...
          if (coAssignee?.email) {
            ...
            await sendTaskAssignedEmail(
              coAssignee.email,
              coAssignee.fullName || coAssignee.username,
              title,
              category || "عام",
              priority || "medium",
              dueDate
  ...
  app.patch("/api/checklist/:id", async (req, res) => {
    ...
    try {
      ...
      // Sanitize update data — convert empty assignedTo to null to avoid ObjectId cast failure
      const updateData: any = { ...req.body };
      if ('assignedTo' in updateData && (!updateData.assignedTo || updateData.assignedTo === '__none__')) {
        updateData.assignedTo = null;
      }
      // Never allow overwriting ownership fields via PATCH body
      delete updateData.userId;
      delete updateData.assignedBy;

      const item = await (ChecklistItemModel as any).findOneAndUpdate(
        { _id: req.params.id, $or: [{ userId: uid }, { assignedTo: uid }, { sharedWith: uid }] },
        { $set: updateData },
        { returnDocument: "after" }
      )
        .populate("assignedTo", "fullName username")
  ...
  app.delete("/api/checklist/:id", async (req, res) => {
    ...
    try {
      ...
      const user = req.user as any;
      const uid = user._id || user.id;
      await (ChecklistItemModel as any).findOneAndDelete({ _id: req.params.id, $or: [{ userId: uid }, { assignedTo: uid }, { sharedWith: uid }] });
      res.json({ ok: true });
    } catch (err) {
  ...
  // Shared with me
  app.get("/api/checklist/shared-with-me", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { ChecklistItemModel } = await import("./models");
      const user = req.user as any;
      ...
      res.status(500).json({ error: "خطأ في تحميل المشتركة" });
    }
  });

  // My personal project groups
  app.get("/api/checklist/my-projects", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { ChecklistItemModel } = await import("./models");
      const user = req.user as any;
      const uid = user._id || user.id;
      const groups = await (ChecklistItemModel as any).distinct("personalGroup", { userId: uid, personalGroup: { $exists: true, $ne: "" } });
      res.json(groups.filter(Boolean));
    } catch (err) {
      res.status(500).json({ error: "خطأ في تحميل المشاريع" });
    }
  });

  // Share / unshare a task with a user
  app.post("/api/checklist/:id/share", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { ChecklistItemModel, UserModel, NotificationModel } = await import("./models");
      const { pushToUser } = await import("./ws");
      ...
      const item = await (ChecklistItemModel as any).findOne({ _id: req.params.id, userId: uid });
      if (!item) return res.status(404).json({ error: "المهمة غير موجودة" });
      if (action === "remove") {
        item.sharedWith = (item.sharedWith || []).filter((id: any) => String(id) !== String(targetUserId));
      } else {
        const already = (item.sharedWith || []).some((id: any) => String(id) === String(targetUserId));
        if (!already) item.sharedWith.push(targetUserId);
      }
      await item.save();
      if (action !== "remove") {
        const target = await (UserModel as any).findById(targetUserId).select("fullName username");
        await NotificationModel.create({ userId: targetUserId, type: "task", title: `🔗 شارك معك ${user.fullName || user.username} مهمة`, body: item.title, link: "/employee/checklist", icon: "🔗" }).catch(() => {});
        pushToUser(String(targetUserId), { type: "task_shared", taskId: String(item._id), taskTitle: item.title, sharedBy: user.fullName || user.username });
      }
      const populated = await (ChecklistItemModel as any).findById(item._id).populate("sharedWith", "fullName username");
      res.json(populated);
    } catch (err) {
      res.status(500).json({ error: "فشل مشاركة المهمة" });
    }
  });

  // Upload attachment for a checklist item
  app.post("/api/checklist/:id/attachment", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { ChecklistItemModel } = await import("./models");
      const user = req.user as any;
      ...
      res.status(500).json({ error: "فشل إضافة المرفق" });
    }
  });

  // Remove attachment from checklist item
  app.delete("/api/checklist/:id/attachment", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
      const { ChecklistItemModel } = await import("./models");
      const user = req.user as any;
      ...
      res.status(500).json({ error: "فشل حذف المرفق" });
    }
  });
...
- كن ودوداً وموجزاً
- تكيّف مع لغة العميل ولهجته تلقائياً`;
      const msgs = [
  ...
  app.post("/api/admin/connection-settings/email", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role !== "admin") return res.sendStatus(403);
    const { emailLogoUrl, emailSiteUrl } = req.body;
  ...
  app.get("/api/admin/client-stores/clients", async (req, res) => {
    ...
    const clients = await (UserModel as any).find({
      role: "client",
  ...
  // /.well-known/apple-developer-merchantid-domain-association — Apple domain verification for Sign In with Apple
  const APPLE_DOMAIN_ASSOCIATION = `_Ai
sN_e-fnOVeP1QgG1I2VBEjv3fEZ9iRNFlUTslpViZpeQAwDZ4K7F2bGcIC2W4IXtb2vTUtODPNQB
IyXp5cbUEdh5qgjC3RVY9e-Kk0sNS-4NtoeTdREQVcsMeAfbN3BGO5f6xOt4KeD07HjjYdpAV4AH
u4icpcdJbcgm05UfTSGijWhzgx7mWVqFllVUsJUuJdx3DWGHgY2JpAN7PAB3LIlqWdNkRNl0pVuK
sVJhX24EMNTz4hA0DJWMS-F71iuFg_InOY1wCCPiFIj_k_QtbUwm4os3hi0CAwEAAaOB5TCB4jAM
...
vLXF6b38y9Ce3JSwHvghlFz_CS4wDgYDVR0PAQH_BAQDAgeAMA8GCSqGSIb3Y2QGOgQCBQAwDQYJ
KoZIhvcNAQELBQADggEBADI0wul3ql_gxsqi83dZ54pnuPFR8_uw9pe_sRGj4aE8uyOS6RKTonEd
vPGacW-kPG82krbgR4Kik-PnuI-73yVEYgLPzbz3-42KCXB4ZcIZTSXLcmIh5Klo-RCaLnoPKL6m
AwbRVWEfr3z4lNRxDuLTJVSLzq3VaAdbvS17x2JFebmph0z4GDuArhBLcdh4K-YKr5rn2U3M6lu3
o5dVa-wNoHjHwLDPy9wQTDCSE3GU1q_g7MnpyZvOJTLuEQ0hFySL8ZUuImJGRX_g29cWVMG5PtPa
irll9rS0I394XdlydmRjpwhVx9m3lNsjv_OTp9QEREMNyuJWsiuUKKQ9cocwggREMIIDLKADAgEC
...
MRMwEQYDVQQKDApBcHBsZSBJbmMuMQswCQYDVQQGEwJVUwIIPVn4N-rR2J8wCQYFKw4DAhoFAKCB
3DAYBgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNjA0MTAxMTQxMjFa
MCMGCSqGSIb3DQEJBDEWBBTHxZIMoWAq2HgllE7OncULvBboVDApBgkqhkiG9w0BCTQxHDAaMAkG
...
hvcNAQEBBQAEggEAszmQ3dU086e4kyegbI_vk0llNN8DjKw_rpEUXNV4hAIs7riU_wOzW28ogyBm
oq3-KpoFu26r_RssFmpbdRWaf8sVIR1l0W-ICK4qNjewI6ct1EZEvX7JYLfAZaRZbPbmsQLK1nxA
BG5zP1d26wqph5DCZkFSnIOFsHf_qKkOevixV6tDDVHvdVoIWe5kxGMqEsGLOCOOgc2H05wux2iO
xIYefHeLwjUtlCijWVnPQjZyU25VuWG0-4IEnWJYRIAOdUEy2ERv8WuujzPXFJo7WnN3zzYmTBI6
sUpy4laxfcJWSuKqtIMN_78SK0eZ9tMHqkrk6EC_-oiHnxkkofFupg`;

  app.get("/.well-known/apple-developer-merchantid-domain-association", (_req, res) => {
    res.setHeader("Content-Type", "text/plain");
    res.send(APPLE_DOMAIN_ASSOCIATION);
  });
  ...
  // GET /api/my-kanban-tasks — tasks assigned to me (any non-client employee)
  app.get("/api/my-kanban-tasks", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    try {
      const { KanbanTaskModel, TaskModel, ProjectModel } = await import("./models");
      const userId = (req.user as any)._id || (req.user as any).id;

      // Fetch from both task systems in parallel
      const [kanbanTasks, projectTasks] = await Promise.all([
        // 1) Standalone admin kanban tasks
        (KanbanTaskModel as any)
          .find({ assignedTo: userId })
          .sort({ createdAt: -1 })
          .populate("assignedTo", "fullName username")
          .populate("createdBy", "fullName username")
          .lean(),
        // 2) Project-level tasks (TaskModel) assigned to this employee
        (TaskModel as any)
          .find({ assignedTo: userId })
          .sort({ createdAt: -1 })
          .populate("assignedTo", "fullName username")
      ...
      // Enrich project tasks with a human-readable project name and normalise shape.
      // Project has no `name` field — resolve label from the linked Order (businessName/orderNumber).
      const projectIds = [...new Set(projectTasks.map((t: any) => String(t.projectId)).filter(Boolean))];
      let projectMap: Record<string, string> = {};
      if (projectIds.length > 0) {
        const projects = await (ProjectModel as any)
          .find({ _id: { $in: projectIds } })
          .select("orderId")
          .populate("orderId", "businessName orderNumber serviceType")
          .lean();
        projects.forEach((p: any) => {
          const order = p.orderId;
          const label = order?.businessName
            || (order?.orderNumber ? `طلب ${order.orderNumber}` : null)
            || order?.serviceType
            || "مشروع";
          projectMap[String(p._id)] = label;
        });
      }

      const normalizedProjectTasks = projectTasks.map((t: any) => ({
        ...t,
        _source: "project",
        projectName: projectMap[String(t.projectId)] || "مشروع",
        // map dueDate → deadline so the UI works without changes
        deadline: t.deadline || t.dueDate || null,
        // map status — project tasks use pending/in_progress/done
        status: t.status || "pending",
        priority: t.priority || "medium",
      }));

      const allTasks = [
        ...kanbanTasks.map((t: any) => ({ ...t, _source: "kanban" })),
  ...
  app.post("/api/admin/kanban/tasks", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    try {
      const { KanbanTaskModel, NotificationModel } = await import("./models");
      const me = req.user as any;
      ...
      if (assignedTo) {
        try {
          const { pushToUser } = await import("./ws");
          const creatorName = me.fullName || me.username || "المدير";
          await NotificationModel.create({
            userId: assignedTo,
            type: "task",
            title: `✅ مهمة جديدة: ${title.trim()}`,
            body: `عيّنها لك ${creatorName}${deadline ? ` · الموعد: ${new Date(deadline).toLocaleDateString("ar-SA")}` : ""}`,
            link: "/admin/kanban",
            icon: "✅",
            read: false,
          });
          pushToUser(String(assignedTo), {
            type: "notification",
            title: `✅ مهمة جديدة: ${title.trim()}`,
  ...
  app.patch("/api/admin/kanban/tasks/:id/status", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    try {
      const { KanbanTaskModel, UserModel } = await import("./models");
      const { status } = req.body;
      const allowed = ["new","under_study","pending_payment","in_progress","testing","review","delivery","closed"];
      if (!allowed.includes(status)) return res.status(400).json({ error: "حالة غير صالحة" });
      const oldTask = await (KanbanTaskModel as any).findById(req.params.id).lean();
      const task = await (KanbanTaskModel as any).findByIdAndUpdate(req.params.id, { status }, { returnDocument: "after", new: true })
        .populate("assignedTo", "fullName username email").lean();
      res.json(task);
      // Send email and notification to assigned employee
      if (task && task.assignedTo) {
        ...
        try {
          const { pushToUser } = await import("./ws");
          if (assignedUser._id) {
            pushToUser(String(assignedUser._id), {
            ...
            fireNotify(
              ...
              { type: 'task', link: '/admin/kanban', icon: '📋' }
            ).catch(() => {});
            if (assignedUser.email) {
              sendTaskStatusEmail(assignedUser.email, assignedUser.fullName || assignedUser.username, task.title, task.projectName || '—', oldStatus, status).catch(console.error);
            }
          }
        } catch (_) {}
      }
  ...
  // PATCH /api/admin/kanban/tasks/:id — edit full task details
  app.patch("/api/admin/kanban/tasks/:id", async (req, res) => {
    if (!req.isAuthenticated() || (req.user as any).role === "client") return res.sendStatus(403);
    try {
      const { KanbanTaskModel } = await import("./models");
      ...
      if (devPlan !== undefined) update.devPlan = devPlan;
  ...
  app.post("/api/admin/notifications/broadcast", async (req, res) => {
    ...
    try {
      ...
      // Also send browser push notifications (fire-and-forget)
      users.forEach((u: any) => {
        sendPushToUser(String(u._id), {
          title,
          body: body || "",
          icon: "/icon-192.png",
          badge: "/favicon-32.png",
  ...
  app.post("/api/internal/push/promotional", async (req, res) => {
    const isInternalCron = req.headers["x-internal-cron"] === INTERNAL_CRON_SECRET;
    if (!isInternalCron) {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      const user = req.user as any;
      if (!["admin", "manager"].includes(user?.role)) return res.sendStatus(403);
...
اكتب الناتج النهائي فقط بدون مقدمة شرح، وبدون Markdown، وبلغة عربية رسمية مفهومة.`;
  ...
  app.get("/api/admin/orders/export", async (req, res) => {
    ...
    try {
      ...
      const orders = await (OrderModel as any).find(filter).sort({ createdAt: -1 }).limit(5000)
        .populate("userId", "fullName username email").lean();
      const headers = ["رقم الطلب", "العميل", "البريد", "نوع المشروع", "القطاع", "المبلغ الإجمالي", "الحالة", "الأولوية", "الإنجاز %", "طريقة الدفع", "تاريخ الطلب", "ملاحظات داخلية"];
  ...
  // ─── Corporate Mail Routes (see server/routes-mail.ts) ─────────────────────
...
export async function seedDatabase() {
  ...
  const needsReseed =
    ...
    (restaurantPro && Number((restaurantPro as any).annualPrice) !== 1699);
    ...
    const SEGMENTS = [
      {
        segment: "restaurant",
        plans: [
          { tier: "lite", monthly: 99, sixmo: 399, annual: 899, lifetime: 5299, popular: false,
          ...
          { tier: "pro",     monthly: 299, sixmo: 799, annual: 1699, lifetime: 9299, popular: true,
          ...
          { tier: "infinite", monthly: 749, sixmo: 1699, annual: 3299, lifetime: 17299, popular: false,
      ...
      },
      {
        segment: "ecommerce",
        plans: [
          { tier: "lite", monthly: 229, sixmo: 649, annual: 1349, lifetime: 7599, popular: false,
            featuresAr: ["متجر إلكتروني أساسي", "حتى 100 منتج", "سلة تسوق", "إدارة الطلبات", "ربط سوشيال ميديا", "SSL مجاني", "دعم فني شهرين"] },
          { tier: "pro",     monthly: 529, sixmo: 1249, annual: 2399, lifetime: 12799, popular: true,
          ...
          { tier: "infinite", monthly: 1099, sixmo: 2399, annual: 4499, lifetime: 23799, popular: false,
      ...
      },
      {
        segment: "education",
        plans: [
          { tier: "lite", monthly: 349, sixmo: 899, annual: 1749, lifetime: 9599, popular: false,
            featuresAr: ["منصة تعليمية أساسية", "حتى 5 كورسات", "إدارة الطلاب", "اختبارات بسيطة", "شهادات إتمام", "SSL مجاني", "دعم فني شهرين"] },
          { tier: "pro",     monthly: 749, sixmo: 1699, annual: 3199, lifetime: 16799, popular: true,
          ...
          { tier: "infinite", monthly: 1449, sixmo: 3099, annual: 5799, lifetime: 29799, popular: false,
      ...
      },
      {
        segment: "corporate",
        plans: [
          { tier: "lite", monthly: 529, sixmo: 1249, annual: 2399, lifetime: 12799, popular: false,
            featuresAr: ["موقع شركة احترافي", "صفحات تعريفية", "نموذج تواصل", "خريطة المكاتب", "SSL مجاني", "دعم فني شهرين"] },
          { tier: "pro",     monthly: 1199, sixmo: 2599, annual: 4899, lifetime: 25299, popular: true,
          ...
          { tier: "infinite", monthly: 2599, sixmo: 5399, annual: 9999, lifetime: 50799, popular: false,
    ...
    app.post("/api/v1/ai/chat", async (req, res) => {
      ...
      try {
        if (doStream) {
          ...
          const settings: any = await QiroxAISettingsModel.findOne({ singleton: "main" }) || {};
          const useLocalAI = process.env.AI_PROVIDER !== "external" || settings.useLocalAI === true;
          const topK = settings.topK || 5;
          const liveCtx = await ai.fetchLiveContext(messages[messages.length-1]?.content || "").catch(() => "");
          let ragDocs = 0;
