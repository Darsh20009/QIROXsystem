...
اجعل الفكرة أكثر وضوحاً ودقة من الناحية التقنية وتجارية. لا تضف قائمة أو نقاط — فقرة واحدة متماسكة.`;
  ...
  app.post("/api/ai/kimi", async (req, res) => {
    ...
    const { messages, systemPrompt, model = "moonshot-v1-32k" } = req.body;
    try {
      const response = await fetch("https://api.moonshot.ai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
...
- رد دائماً بالعربية إلا إذا تكلم العميل بلغة أخرى`;
      ...
      const completion = await groq.chat.completions.create({ model: "moonshot-v1-32k", messages: msgs, max_tokens: 400, temperature: 0.7 });
