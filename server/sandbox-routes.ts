...
const SECRET_SCAN_EXTENSIONS = new Set([
  ".js", ".jsx", ".ts", ".tsx", ".mjs", ".cjs", ".json", ".env", ".example",
  ".yaml", ".yml", ".toml", ".py", ".rb", ".go", ".rs", ".php", ".java",
]);

function secretSuggestionType(key: string): "local" | "external" | "config" {
  const normalized = key.toUpperCase();
  if (LOCAL_SECRET_KEYS.has(normalized)) return "local";
  if (/(API|TOKEN|PASSWORD|PRIVATE|CLIENT_SECRET|ACCESS_KEY|SECRET|DATABASE_URL|MONGODB_URI|OAUTH)/.test(normalized)) return "external";
  return "config";
}

async function inspectProjectEnvironment(projectId: string, existingKeys: Set<string>) {
  const { getProjectDir } = await import("./sandbox-fs");
  const fs = await import("fs/promises");
  const path = await import("path");
  const root = getProjectDir(projectId);
  const detected = new Map<string, { key: string; type: "local" | "external" | "config"; files: string[] }>();
  let filesRead = 0;
  let bytesRead = 0;
  const maxFiles = 180;
  ...
  const add = (key: string, filePath: string) => {
    ...
    if (!/^[A-Z][A-Z0-9_]{2,63}$/.test(normalized)) return;
    const previous = detected.get(normalized) || {
      key: normalized,
      type: secretSuggestionType(normalized),
      files: [],
    };
    if (!previous.files.includes(filePath) && previous.files.length < 4) previous.files.push(filePath);
    detected.set(normalized, previous);
  };

  const scanContent = (content: string, filePath: string) => {
    const patterns = [
      /process\.env\.([A-Z][A-Z0-9_]{2,63})/g,
      /process\.env\[['"]([A-Z][A-Z0-9_]{2,63})['"]\]/g,
      /import\.meta\.env\.([A-Z][A-Z0-9_]{2,63})/g,
      /import\.meta\.env\[['"]([A-Z][A-Z0-9_]{2,63})['"]\]/g,
      /os\.getenv\(\s*['"]([A-Z][A-Z0-9_]{2,63})['"]/g,
      /System\.getenv\(\s*['"]([A-Z][A-Z0-9_]{2,63})['"]/g,
    ];
    for (const pattern of patterns) {
      for (const match of content.matchAll(pattern)) add(match[1], filePath);
    }
    if (filePath.toLowerCase().includes(".env")) {
      for (const line of content.split(/\r?\n/)) {
        const match = line.match(/^\s*([A-Z][A-Z0-9_]{2,63})\s*=/);
  ...
  const walk = async (directory: string, relative = ""): Promise<void> => {
    if (filesRead >= maxFiles || bytesRead >= maxBytes) return;
    let entries: any[] = [];
    try { entries = await fs.readdir(directory, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (filesRead >= maxFiles || bytesRead >= maxBytes) break;
      if (entry.name.startsWith(".git") || ignoredDirectories.has(entry.name)) continue;
      ...
      if (!SECRET_SCAN_EXTENSIONS.has(extension) && !entry.name.startsWith(".env")) continue;
      try {
        const stat = await fs.stat(absolute);
        if (!stat.isFile() || stat.size > 120_000) continue;
        const remaining = Math.max(0, maxBytes - bytesRead);
        const content = (await fs.readFile(absolute, "utf8")).slice(0, remaining);
        bytesRead += content.length;
        filesRead++;
        scanContent(content, relativePath);
      } catch { /* a single unreadable file must not block the scan */ }
    }
  };
  ...
  return Array.from(detected.values())
    ...
    .map(item => ({
      ...item,
      existing: existingKeys.has(item.key),
      canGenerate: item.type === "local",
    }));
}
...
async function requireProjectAccess(req: Request, res: Response): Promise<{ user: any; project: any } | null> {
  ...
  const isOwner = String(project.ownerId) === String(user._id || user.id);
  let isProjectDeveloper = false;

  // A workspace created from a client project belongs to the workspace owner,
  // but its assigned developers/designers must be able to work in it too.
  // Membership is checked against the source Project, not against a client-
  // supplied workspace id, so access cannot be granted by guessing an id.
  if (!isOwner && !isAdmin && ["developer", "designer"].includes(user.role) && project.sourceProjectId) {
    const { ProjectMemberModel, ProjectModel } = await import("./models");
    const userId = String(user._id || user.id);
    const [membership, managedProject] = await Promise.all([
      ProjectMemberModel.exists({ projectId: project.sourceProjectId, userId }),
      ProjectModel.exists({ _id: project.sourceProjectId, managerId: userId }),
    ]);
    isProjectDeveloper = Boolean(membership || managedProject);
  }

  if (!isOwner && !isAdmin && !isProjectDeveloper) {
...
export function registerSandboxRoutes(app: Express, httpServer?: HttpServer): void {
  ...
  app.get("/api/sandbox/projects", async (req: Request, res: Response) => {
    ...
    if (!user) return;
    try {
      const { SandboxProjectModel, ProjectMemberModel, ProjectModel } = await import("./models");
      const { isRunning } = await import("./sandbox-runner");
      const isAdmin = ["admin", "manager"].includes(user.role);
      let query: any;
      if (isAdmin) {
        query = {};
      } else if (["developer", "designer"].includes(user.role)) {
        const userId = String(user._id || user.id);
        const [memberships, managedProjects] = await Promise.all([
          ProjectMemberModel.find({ userId }).select("projectId").lean(),
          ProjectModel.find({ managerId: userId }).select("_id").lean(),
        ]);
        const sourceProjectIds = [
          ...memberships.map((entry: any) => entry.projectId),
          ...managedProjects.map((entry: any) => entry._id),
        ];
        query = {
          $or: [
            { ownerId: user._id || user.id },
            { sourceProjectId: { $in: sourceProjectIds } },
          ],
        };
      } else {
        query = { ownerId: user._id || user.id };
      }
  ...
  app.patch("/api/sandbox/projects/:id", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const allowed = isAdmin
        ? ["name", "nameAr", "description", "entryFile", "startCmd", "installCmd", "buildCmd", "runtime", "githubRepo", "githubBranch", "isPublic", "tags", "logoUrl"]
      ...
      if (updates.logoUrl !== undefined) {
        const normalizedLogoUrl = normalizeLogoUrl(updates.logoUrl);
        if (String(updates.logoUrl || "").trim() && !normalizedLogoUrl) {
          return res.status(400).json({ error: "رابط الشعار يجب أن يكون http أو https أو مساراً محلياً صالحاً" });
        }
      ...
      // Keep the public deployment record in sync with the Workspace identity.
      // The client updates both records when the deployment panel is open, but
      // this server-side sync also covers existing deployment projects and
      // edits made from another Workspace surface.
      if (updates.logoUrl !== undefined && updated?.githubRepo) {
        const repo = parseGithubRepo(updated.githubRepo);
        if (repo) {
          const { DeploymentProjectModel } = await import("./models");
          await DeploymentProjectModel.updateMany(
            {
              ownerId: String(updated.ownerId),
              githubOwner: repo.owner,
              githubRepo: repo.repo,
            },
            { $set: { logoUrl: updates.logoUrl } },
          );
        }
      }
  ...
  app.post("/api/sandbox/projects/:id/command", async (req: Request, res: Response) => {
    const ctx = await requireProjectAccess(req, res);
    if (!ctx) return;
    try {
      ...
      const { recordSandboxLog } = await import("./sandbox-runner");
      const env: Record<string, string> = { ...(process.env as Record<string, string>), PORT: String(ctx.project.port || "") };
      ...
      child.stdout?.on("data", (data: Buffer) => recordSandboxLog(String(ctx.user._id), String(ctx.project._id), "stdout", data.toString()));
      child.stderr?.on("data", (data: Buffer) => recordSandboxLog(String(ctx.user._id), String(ctx.project._id), "stderr", data.toString()));
      const timeout = setTimeout(() => {
        try { child.kill("SIGTERM"); } catch {}
        recordSandboxLog(String(ctx.user._id), String(ctx.project._id), "stderr", "\n⏱ انتهت مهلة الأمر (120 ثانية)\n");
      }, 120000);
      child.on("exit", (code) => {
        clearTimeout(timeout);
        recordSandboxLog(String(ctx.user._id), String(ctx.project._id), code === 0 ? "stdout" : "stderr", `\n⏹ انتهى الأمر (كود: ${code})\n`);
      });
      res.json({ success: true, command: safeCommand });
    } catch (err: any) {
      res.status(400).json({ error: err.message || "الأمر غير مسموح" });
    }
  });
  ...
  app.post("/api/sandbox/projects/:id/build", async (req: Request, res: Response) => {
    ...
    try {
      ...
      recordSandboxLog(String(ctx.user._id), pid, "stdout", `Running build: ${safeBuildCmd}`);
      ...
      buildProcess.stdout?.on("data", (d: Buffer) => recordSandboxLog(String(ctx.user._id), pid, "stdout", d.toString()));
      buildProcess.stderr?.on("data", (d: Buffer) => recordSandboxLog(String(ctx.user._id), pid, "stderr", d.toString()));
      buildProcess.on("exit", (code) => {
        if (code === 0) {
          recordSandboxLog(String(ctx.user._id), pid, "stdout", "Build completed successfully!");
        } else {
          recordSandboxLog(String(ctx.user._id), pid, "stderr", `Build failed with exit code ${code}`);
        }
      });
      buildProcess.on("error", (err) => {
        recordSandboxLog(String(ctx.user._id), pid, "stderr", `Build error: ${err.message}`);
      });
  ...
  app.get("/api/sandbox/projects/:id/status", async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.json({
        projectId: pid,
        isRunning: running,
        port: running ? (proc?.port || ctx.project.port) : null,
        pid: proc?.pid || null,
  ...
  app.get("/api/sandbox/projects/:id/logs", async (req: Request, res: Response) => {
    ...
    if (!ctx) return;
    try {
      const { getProcessInfo, getProcessLogs } = await import("./sandbox-runner");
      const pid = String(ctx.project._id);
      const proc = getProcessInfo(pid);
      const logs = getProcessLogs(pid);
  ...
  app.get("/api/sandbox/projects/:id/env/inspect", async (req: Request, res: Response) => {
    const ctx = await requireProjectAccess(req, res);
    if (!ctx) return;
    try {
      const { SandboxEnvVarModel } = await import("./models");
      const existing = await SandboxEnvVarModel.find({ projectId: ctx.project._id }).select("key").lean();
      const existingKeys = new Set((existing as any[]).map((item) => String(item.key || "").toUpperCase()));
      const suggestions = await inspectProjectEnvironment(String(ctx.project._id), existingKeys);
      res.json({
        project: {
          runtime: ctx.project.runtime,
          template: ctx.project.template,
          buildCmd: ctx.project.buildCmd,
          startCmd: ctx.project.startCmd,
        },
        suggestions,
      });
  ...
  app.post("/api/sandbox/projects/:id/env/generate", async (req: Request, res: Response) => {
    const ctx = await requireProjectAccess(req, res);
    if (!ctx) return;
    try {
      const key = String(req.body?.key || "").trim().toUpperCase();
      if (!LOCAL_SECRET_KEYS.has(key)) {
        return res.status(400).json({ error: "لا يمكن توليد مفاتيح مزودي الخدمات تلقائياً. أدخلها من لوحة المزود." });
      }
      const { SandboxEnvVarModel } = await import("./models");
      const { encrypted, iv } = encrypt(crypto.randomBytes(48).toString("base64url"));
      await SandboxEnvVarModel.findOneAndUpdate(
        { projectId: ctx.project._id, key },
        { value: encrypted, iv },
        { upsert: true, new: true },
...
لا تستخدم markdown code fences.`;
  ...
  app.get("/api/sandbox/projects/:id/download", async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Sync files from DB to disk before archiving
      await syncDbToDisk(pid, ctx.project._id);

      const archiver = (await import("archiver")).default;
      const { getProjectDir, ensureProjectDir } = await import("./sandbox-fs");
      ensureProjectDir(pid);
  ...
  app.post("/api/sandbox/projects/:id/import-zip", async (req: Request, res: Response) => {
    ...
    try {
      ...
      upload(req as any, res as any, async (err: any) => {
        ...
        const { safePath, ensureProjectDir } = await import("./sandbox-fs");
        ensureProjectDir(pid);
        ...
        for await (const entry of zip) {
          ...
          if ((entry as any).type === "File" && !filePath.includes("node_modules/") && !filePath.includes(".git/")) {
            const buf: Buffer = await (entry as any).buffer();
            totalSize += buf.length;
            if (totalSize > MAX_TOTAL_SIZE) break;
            const absPath = safePath(pid, filePath);
  ...
  app.post("/api/sandbox/projects/:id/github/push", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (git && safeRepoUrl) {
        try { await git.remote(["set-url", "origin", safeRepoUrl]); } catch { /* ensure PAT removed */ }
      }
    }
  });
  ...
  // ── Route Aliases (/api/sandbox/:id/... → /api/sandbox/projects/:id/...) ──
  app.use("/api/sandbox/:id", (req: Request, res: Response, next: any) => {
    const sub = (req as any).path || "";
    if (sub === "" || sub === "/") return next();
    const reserved = ["/projects", "/admin", "/templates"];
    if (reserved.some((r) => sub.startsWith(r))) return next();
    const qs = req.originalUrl.includes("?") ? "?" + req.originalUrl.split("?")[1] : "";
    req.url = `/api/sandbox/projects/${req.params.id}${sub}${qs}`;
