...
- ⏳ **Pending:** end-to-end verification against real data (flag ON + authenticated session + live Mongo documents) requires `MONGODB_URI`, which is not configured in this environment (flagged separately at project import — this is a pre-existing environment gap, not introduced by Sprint C). Code paths were verified by direct reading against the real Mongoose schemas (`server/models/{orders,finance,projects,comms}.ts`) for field-name correctness. Once `MONGODB_URI` is available, re-run the same curl checks with a real session cookie and `FEATURE_CUSTOMER_JOURNEY_V2=true` to confirm live-data responses.
