...
// All API calls are proxied through our Express server at /cafe-api
// so the browser never makes cross-origin requests (avoids CORS credential issues)
const API_BASE = "/cafe-api";
...
async function tryRestoreSession(): Promise<boolean> {
  ...
  try {
    const employee = JSON.parse(stored);
    const res = await fetch(`${API_BASE}/api/employees/restore-session`, {
      method: "POST",
